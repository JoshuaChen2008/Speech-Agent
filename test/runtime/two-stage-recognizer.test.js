...
test('SEM-F21/J16 publishes draft partials until authoritative recognition owns the segment', () => {
  ...
  assert.equal(adapter.poll(), null, 'draft cannot write back after authoritative takeover')
...
test('SEM-F21/J16 fails closed with the frozen start code when draft construction fails', () => {
...
test('SEM-F21/J16 releases a valid draft when authoritative construction is invalid', () => {
...
test('SEM-F21/J16 degrades once on draft runtime failure and keeps authoritative final', () => {
  const faults = []
...
test('SEM-F21/J16 propagates authoritative failures and releases both streams once', () => {
  const draft = scriptedAdapter()
