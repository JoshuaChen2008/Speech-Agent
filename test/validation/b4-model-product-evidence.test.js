...
test('Windows CI supervises the full four-window product journey and verifies both reports', () => {
  ...
  assert.match(workflow, /verify-product-shell-report\.js "\$profileRoot\/report\.json"/)
  assert.match(workflow, /verify-electron-exit-evidence\.js "\$profileRoot\/exit-evidence\.json"/)
  assert.match(workflow, /@\('default', 'legacy-risk', 'current-risk'\)/)
})
