...
| **B3.3 SQLite 字幕历史（进行中）** | storage worker、SQLite 字幕事件/segments、JSONL 迁移、历史查询 API | 开发态 DB0、DB1 与 Gateway/Recorder/Coordinator 恢复组合已通过：open ACK 前不采集、close ACK 前不 idle、空闲/提交前/提交后 worker 故障可幂等换代；默认 `main.js` 仍只写 JSONL，下一步完成 DB2/J10、冷启动残留会话/`before-quit`、一次性权威切换和历史 UI；打包态 DB0 仍待 B5/I4 |
