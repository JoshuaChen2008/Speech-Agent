...
| 锁定穿透、逐像素命中测试、主进程手动拖动 | 历史面板 UI（SQLite 产品数据层仍待接入） |
...
| B3 **二遍精修 refine worker**（final→refined 自动变准补标点）+ JSONL 过渡会话档；B3.3 DB0/DB1 SQLite worker 基座 | B3.3 产品网关接线、旧 JSONL 迁移、权威切换与历史 UI |
...
| **B3.3 SQLite 字幕历史（进行中）** | storage worker、SQLite 字幕事件/segments、JSONL 迁移、历史查询 API | 开发态 DB0 与 DB1 原子/幂等门禁已通过（打包态 DB0 仍待 B5/I4）；下一步接入 gateway/recorder 与 worker 自动恢复，再完成 DB2/J10；J1/J2 切到 SQLite 后覆盖“停止/重启→历史查看”；迁移前后 final/refined 原文投影与 txt/md/srt 原文 digest 一致；遗留 translated 不进入字幕事实；禁止长期 JSONL 双写 |
