...
> 本次修订的历史 B5 本地快照：J15a 固定高度逐行淘汰、J15b 不可变原始版/独立精修版和 J15c 精修可选化主体已达到联合验收完成。旧产品载荷的未签名 installer SHA-256 为 `d862c5fca0e477abc2d636573e1dd41aef564ed0c82def3bd469966db7b0de10`；packaged test exe 首轮完成核心/精修分层、会话冻结、故障回退/工具条会话状态通知、跨会话版本导出和旧档迁移，第二进程在模型 fetch=0 下恢复两组 ready、偏好、历史和故障事实且不重放旧提示，两轮 clean exit。同一 run ID、四份报告 SHA 与 114 文件历史产品载荷 SHA `a1f03ed6…9accc` 已绑定到当时的正式 release layout。NSIS 机械探针只证明安装/卸载 exit 0、安装目录移除及无关 APPDATA 哨兵不变，不冒充正式应用 `userData` 保留。I3 非音频预资格覆盖 3,600 段/4,000 事件、虚拟两小时、72 页 DOM≤50、WAL/查询/CPU/内存/队列、三格式导出和重开恢复，但固定为 `pass/partial`，不冒充真实两小时音频 soak。I4 非音频专用机 runner、fixture、严格 verifier 与回归契约已完成；当前开发机不满足干净 profile 条件，故尚无执行报告，不能据此关闭 I4。
>
...
>
> 2026-08-03 远端状态纠偏：run `30750568366` 在 revision `2242103eb917f2afbfe81c7c8df788852bb36ebc` 因 Electron runtime 未供给而止于首个字幕布局步骤；run `30760407160` 在 revision `0d0cd9f91cfd5136bbd5d3fec44e636da04b4e21` 通过 runtime、字幕布局与 DB0 后暴露 DB1 fixture/版本隔离漂移及 hidden artifact 未上传。run `30761472817` 暴露模型 fixture 与十个 I3 文本输入的可移植性缺口；run `30763123116` 暴露完整产品文本 checkout 行尾漂移；run `30764235663` 暴露 I3 fixture 的外部时区边界。修复后，run `30765231206` 精确绑定 revision `3b4b0db2f56f57e3d0e7500c77593b406a5dd857`，完整 workflow、provenance 绑定与 artifact 上传成功；artifact 下载后 strict reader 因 caption layout runner/verifier 在 hosted checkout 为 CRLF、本机为 LF 而拒绝复算。两份脚本及索引复算的 lockfile/workflow LF checkout 修复由 run `30766172580` 验证；较早的合格基线 run `30768637180` 精确绑定 revision `2d3d6bdf5d745c15c239ef7503a0dfac211409a8`，全部 workflow steps 与 609 项回归成功。artifact `qualification-evidence-2d3d6bdf5d745c15c239ef7503a0dfac211409a8-30768637180-1`（ID `8839812004`，GitHub ZIP digest `3663225294fbd1019ab6e0e1e29608297fa767eb6c8e4b9012972fcd2d1f16c6`）下载后通过五个 strict readers、四份报告与 lockfile/workflow SHA、跨报告绑定及 41 文件/29 JSON 隐私负扫描。I3 修复代码候选 run `30784483976` 精确绑定 revision `82d56f64c80c74f30c1944665460f1316f1d7939`，全部 workflow steps 与 627 项回归成功；artifact `qualification-evidence-82d56f64c80c74f30c1944665460f1316f1d7939-30784483976-1`（ID `8844827701`，GitHub ZIP digest `78227ef5b4af08f3f2319156cb4ef096a132599707446506c32124f0ecbcdaca`）下载后通过同一组 strict readers、绑定复核与隐私负扫描，该精确 revision 的 J9-CI 已达到联合验收完成。远端索引绑定的 installer SHA 为 `bdac65ed…a7c4`；下载包不含该 installer 字节，本机 B5 安装器 `d862c5fc…0de10` 仍由本机七份报告独立绑定。 后续证据投影 run `30786324179` 精确绑定 revision `c36aefaee4778a1bf2dfe1ee005924a724f4be53`，通过 Electron、布局、存储、产品壳、packaged 与 NSIS 前置后，在 evidence lane 因 I3 live playback HTML 未固定 LF 而 fail closed，最终 provenance 索引按设计跳过；该 revision 保持实现完成·尚未验收。 LF 修复 run `30787209338` 随后精确绑定 revision `f86ac1ef604dc7da0728c6eda44d59bbfd1e09bf`，全部 workflow steps 与 629 项回归成功；artifact `qualification-evidence-f86ac1ef604dc7da0728c6eda44d59bbfd1e09bf-30787209338-1`（ID `8845725648`，GitHub ZIP digest `b9d0a56b55fa4c6728be660698654bce418ee6364fd43a59eb1be9ccb9993242`）下载后完成严格复核，远端 installer SHA 为 `b9becb19…16f31`，该精确 revision 的 J9-CI 已达到联合验收完成。

> 2026-08-03 最新远端资格：run `30790372286` 精确绑定 I2 模型替换决策 revision `5c6ce847fc07329802e3e98db9db70cc683f1f75`，Electron `43.2.0` 前置，workflow 结论为 `success`；core 414 tests=407 pass+7 expected model/Silero-asset skips，integration 27/27，evidence 190/190；总计 631 tests=624 pass+7 expected skips+0 fail。7 项跳过不计作模型测试成立；artifact `qualification-evidence-5c6ce847fc07329802e3e98db9db70cc683f1f75-30790372286-1`（ID `8846860080`，GitHub ZIP digest `5968779b61db0eb6f6d2d7e7dcaa3d0a38844f8987a43941bb4395aff5ba69ef`）下载后通过五个 strict readers、四份报告与两个源码 digest、跨报告绑定及 41 文件/29 JSON 隐私负扫描。下载包不含 installer 字节，同轮索引绑定 installer SHA `618f02eddbbd3a956d679b17180817665d48dd0b9608262fd6519f53eac857e0`。该精确 revision 的 J9-CI 已达到联合验收完成；Gate 0B 替代模型仍未选定，I2/J15a/I3/I4 实机门禁不变。

> 2026-08-07 文档对齐复核：代码与 evidence 基线 revision `a91a200cc2d3df5f73dcb7a0894a9758e532106a` 在正常 Windows 子进程权限下取得 core 414 tests=407 pass+7 expected skips、integration 27/27、evidence 200/200；总计 641 tests=634 pass+7 expected skips+0 fail；受限沙箱中的 3 项 `tar.exe EPERM` 属 SEM-T03 执行环境问题。最近一次 J9-CI 联合验收完成基线是 run `30792514100` / revision `2890dd9a5a224bca75cff2257cfd14013d74584d`：631 tests=624 pass+7 expected skips+0 fail；artifact ID `8847650958`、GitHub ZIP digest `9e4b2d7b9975afc288c5a14728e0a8812ec00eea1f6a25f46080063c8beccafd`，下载侧五个 strict readers、四份报告与两个源码 digest、跨报告绑定及 41 文件/29 JSON 隐私负扫描闭合；索引绑定 installer SHA `25ff62a88efb8e8d021887df3a2685c72515baadca23f4810902f13d2ae19b76`，下载包不含 installer 字节。当前 revision 的 run `30801570384` 已通过产品与打包前置，但 evidence 200 tests=197 pass+3 fail，最终 provenance 索引按设计跳过；`scripts/gate-0b/corpus.json` 缺少 LF checkout 规则，hosted CRLF SHA `fdf4420a243cc6e0efe0074dbf5c60c97efc1d1284448b7bfce9c3d658e1fce9` 与登记的 LF SHA `7edd6dff286b84619a3b68f385ba04622103ffe17cc57dbe5e1f16521deb156d` 不同，因此当前 revision 的 J9-CI 为实现完成·尚未验收。产品载荷输入自 `2d3d6bdf5d745c15c239ef7503a0dfac211409a8` 起未变化，本机 B5 候选与 SEM-F18/SEM-T12 产品载荷绑定保持不变；J15a 可见矩阵、I2、I3、I4 及 Agent 系统状态均未提升。
...
> 2026-08-30 Agent 重设计排期：Agent 系统的排期与状态改由 [`docs/agent-redesign-execution-plan.md`](docs/agent-redesign-execution-plan.md) 承载，本文件中的 A1/A2、I5、V5 与 §5.4/§6.5 只描述插件时代的旧切分，已由 ADR 0013/0014/0015 取代，不再是排期依据。S1 个人上下文模块骨架（J21）的状态为「实现完成·尚未验收」：v5、新旧表隔离、三接口、exact IPC、scheduler seam 和 J21 S1 失败矩阵均已取证，产品资格保持 `provider_not_configured`，自动路径零运行、零报告，字幕 `open/append/close/history` 不加载任一 Agent store。`npm run test:core` 与 `npm run test:evidence` 返回 0；完整 integration lane 因 Windows Electron GPU 子进程 `exit_code=-1073741515` 返回非零，但 S1 J21 联合测试自身返回 0。完整 J21 与 S2–S5 尚未开始。SQLite 后续变化仍按追加 migration 落地为 v6（S2）、v7（S3），S4/S5/S6 不带 schema 变化，v1–v4 逐字节不变。字幕系统状态不因本轮变化：SEM-F00 要求 Agent 任何失败都不改变字幕显示、SQLite 历史与导出，每片必须附字幕系统零回归证明。旧 Agent 实现留在仓库但不在产品启动路径上；S6 的工作是给它加守卫并补全打包排除，不是关掉正在运行的东西。
...
> **B5 制品边界：**历史本地 exact installer SHA-256 `d862c5fca0e477abc2d636573e1dd41aef564ed0c82def3bd469966db7b0de10` 只绑定旧产品载荷。当前 bbfd/run 资格绑定远端 installer SHA `d77d16c0…060c` 与产品载荷 SHA `e95fd87f…a35a`，但下载 artifact 不含 installer 字节；进入 I4 前必须另行取得并核验该精确安装器，不能用摘要或历史本地候选替代。
...
| **B4 字幕资源（确定性 UI 联合验收完成；I4 待验）** | 内置固定四资源 manifest、ModelManager、断点下载、流式 SHA、固定 System32 tar、归档审查/白名单提取、staging 原子安装、严格 marker、资源页与空闲热启用 | 核心字幕模型资源包由临时字幕识别器、权威识别器与 VAD 三项 marker 组成；可选精修使用独立 marker。设置页提供三项核心角色、核心下载、精修独立下载/取消/继续与全局偏好；schema-v4 产品壳及 packaged 首启/复启闭合 3 个核心 marker / 4 项总资源。真实张量、公网资源与干净机仍归 I2/I4。 |
| **B5 字幕 MVP 分发（确定性范围联合验收完成；I4 待验）** | electron-builder 26.15.3、ASAR allowlist/fuses、native unpack、packaged 双启动、NSIS、CI provenance | 当前 run `31191838016` 精确绑定 revision `bbfd7041e5963e51942392323735298a7b81cb30`：core 422 tests=415 pass+7 expected model/Silero-asset skips、integration 29/29、evidence 204/204；总计 655 tests=648 pass+7 expected skips+0 fail。artifact ID=`8999273285`、ZIP digest=`5ce4070c…55af`、远端 installer SHA=`d77d16c0…060c`、产品载荷 SHA=`e95fd87f…a35a`；下载 artifact 不含 installer 字节。schema-v4 产品壳与 packaged 首启/复启闭合三项核心 marker、四项总资源及独立精修资源。历史本地未签名 NSIS `d862c5fc…0de10` 与产品载荷 `a1f03ed6…9accc` 只由其七份旧 B5 报告绑定，不得作为当前 I4 候选；I4 前必须另行取得并核验 bbfd/run 的精确 installer 字节。该确定性资格不关闭真实张量、物理来源、DWM、I2/I3 或 I4 实机门禁。 |
| **B6 固定高度字幕流与转写版本（确定性主链完成）** | J15a：字幕窗使用固定 bounds 的连续字幕流（CSS 底部锚定 + 顶部整行淘汰，容器高度取整到整行）；最后一条视觉行退出后，renderer 只回报 schema/会话/段身份，主进程从 canonical 实时视图永久淘汰该段，本会话迟到修订、故障回退、窗口放大和 reload 均不得复活。J15b：首次 `final` 与精修稿版本隔离，历史详情一次返回两版、导出按版本参数分别核对 digest；版本选择按当前会话作用，切换会话重置原始版。J15c：核心/精修资源拆分；全局精修偏好只影响未来新会话且不删除旧稿；下载支持明确取消/继续；迟到精修只更新仍可见 final；worker 故障确认时立即恢复所有仍可见 final、保持当前 partial 和固定 bounds，本会话不重启/补跑且不修改全局偏好；正常停止后工具条显示无抢焦点的会话状态通知并链接历史，该通知只报告处理状态、不概括或改写字幕内容，保持到关闭/进入历史并在新会话清除、重启不重放；独立五值故障事实和整场 `N/M` 跨重启保留，`N=M` 不掩盖故障；空会话、旧会话和异常退出分别保留明确语义；不完整回退统一标记 `[原始版回退]`；日志按 5×1 MiB/7 天本地滚动且不自动上传 | J15a/J15b/J15c 已达到联合验收完成：固定整行视口、逐行淘汰、identity-only viewport eviction、canonical replacement/reload 与迟到修订/回退不复活有 reducer、IPC、真实 Chromium 和产品壳证据；首次 `final` 锚定、双版本 digest、会话作用域选择和真实 packaged 跨会话导出已完成；核心/精修分层、全局偏好与会话冻结、故障恢复、schema v2 会话结果、`N/M`、空/零/旧会话语义、工具条会话状态通知和滚动日志均有局部及跨模块证据。真实 packaged 双启动覆盖下载中取消、连接关闭、合法 `.part` 保留、复启 fetch=0、明确 Range 继续、故障静默期、main→IPC→toolbar 通知、历史复读与不重放。DPI/主题/透明窗人工视觉、I4 专用干净机与延期音频门禁尚未达到实机验收完成。CI 继续在失败时上传 `.artifacts` 报告与日志 |
...
| **I2 Live Caption（实现完成·尚未验收）** | 单路音频 → realtime ASR → SessionCoordinator → 字幕 UI | 受跟踪权威 bundle 的 loopback/mic 各五轮均有 schema-v5 child + exact-exit sidecar + schema-v6 series，冻结 P95=1158/1005ms。revision `b96b8fe7db5ba4db3ac36c4ee85371a4381b521f` 的补充 `loopback` 五轮同样闭合首次稳定转写/精修稿、CER、自然退出、零损失与隐私边界，series SHA=`2a365e3c…1b48`；冻结 P50/P95/min/max=1144/1242/1054/1242ms，仍未满足 `<1000ms`。逐轮模型音频需求为 712.625–776.562ms；最慢轮按同轮采集/VAD 前置与触发后预算只允许低于 534.562ms，而 Gate 0B 同语料裸模型最大音频需求为 660ms，因此已决定停止参数微调并重新开启 realtime 模型替换评估，替代模型尚未选定，冻结起点/门槛/Silero/4×100ms/12×100ms 均不变。真实 loopback pause/refine 与 exact worker 强制终止+Retry 已通过；DWM 持续 1,580 帧零损失但无操作者 completion，实际设备移除和 Windows 睡眠/唤醒未执行。mic 仍只是 physical-preferred 标签启发式，且没有同 revision 新五轮。完整 I2 尚缺替代模型、两来源新五轮、原生拖动及设备/睡眠恢复。 |
| **I3 Durable Subtitle Session（实现完成·尚未验收）** | 首次稳定转写/精修稿 → SQLite 事件/投影 → 带时间戳历史/导出 | 3,600 段虚拟两小时已验证资源上界、72 页 DOM、完整导出和重开恢复，固定为 `pass/partial`。revision `82d56f64c80c74f30c1944665460f1316f1d7939` 的 75 秒真实 `loopback` 资格报告 SHA=`0c219b9627618cdda12ad41ae77093fd5f7bcccbe30b042c1c9cad2958d702f4`，在 30 秒强退 realtime worker 后经明确 Retry 恢复，取得恢复前/后/总计 14/17/31 个首次稳定转写和 29 个精修稿；SQLite/导出/资源/transport/worker/storage 恢复检查成立，强退世代计数独立保留，恢复后世代所有损失轴为零，且不保存现场音频。该报告仅为 `pass/partial` 资格；仍需真人原生拖动、7,200 秒和至少 3,000 个首次稳定转写的正式验收。 |
| **I4 Packaged Subtitle MVP（实现完成·尚未验收）** | 精确 NSIS 的干净机首启、公网下载、权限、真实 ASR、持久化、历史、离线复启与卸载数据策略 | 非音频 runner/verifier、来源隔离的 `loopback`/`mic` 音频 child、strict summary 与只含精确安装器/B5/runner/verifier/fixture 的移交包入口现均为实现完成·尚未验收。音频 runner 以操作者 attestation 记录 GUI 行为，并由 harness 独立核对进程串行、安装文件、SQLite、导出、离线复启和隐私边界；只有非音频报告和两个音频 child 全部绑定同一候选并通过 strict summary，才能达到发布验收完成。当前尚无专用干净 Win11 的三份 child 报告。 |
...
> **Superseded 提示：**本节以下两段描述的是 ADR 0004 之前的旧候选。[ADR 0004](docs/adr/0004-immutable-first-pass-and-optional-refinement.md)、`SEM-F17` 与 `SEM-F21` 已接受并实现新的边界——**核心字幕 ready 依赖临时字幕识别器、权威识别器与 VAD；精修模型默认不下载，只由未监听状态下明确用户动作按需安装，缺失或失败只降级精修能力。**当前实现与验收按 J14/J15c/J16 执行。保留下文只为记录旧决定及其证据，**不得据此写测试，也不得再用旧式一次性 bundle 阻塞默认字幕**。
