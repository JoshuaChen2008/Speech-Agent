...
| **B3.3 SQLite 字幕历史（进行中）** | storage worker、SQLite 字幕事件/segments、JSONL 迁移、历史查询 API | 开发态 DB0 已通过（打包态仍待 B5/I4）；后续 DB1–DB2 与 J10 通过；J1/J2 切到 SQLite 后端并覆盖“停止/重启→历史查看”；迁移前后 final/refined 原文投影与 txt/md/srt 原文 digest 一致；遗留 translated 不进入字幕事实；禁止长期 JSONL 双写 |
