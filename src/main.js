...
// 工具条内容宽按最坏情况固定（error 态：状态副行 + 停止 + 重试 + 下一步）。
// 476 = Electron 实测自然宽 459（@125% DPI）+ 17 余量，留给不同 DPI 的字体度量
// 差异和比样例更长的音频源标签。超出余量时状态副行会打省略号，条不会换行。
// 条本身自适应，窗内右对齐，多出的透明区域逐像素穿透。
// dock() 按右沿反推，公式不受宽度影响。
const TB_W = 476 + TB_MARGIN * 2
