...
import * as PC from '../ui/shared/personal-context-ui'

type Pane = 'display' | 'audio' | 'asr' | 'resources' | 'context' | 'about'
type ModelState = 'missing' | 'downloading' | 'verifying' | 'ready' | 'error'
type PersonalContextResource = 'session_episodes' | 'personal_memories'
type Dict = Record<string, any>

const PANES: ReadonlyArray<readonly [Pane, string]> = [
  ['display', '显示与字幕'], ['audio', '音频源'], ['asr', '语音识别'],
  ['resources', '模型资源'], ['context', '个人上下文'], ['about', '关于']
]
...
/* 设置 · 个人上下文。只经 window.shell 的三个冻结入口消费
   speech-agent.personal-context.ui@1.0.0：先订阅 changed，再读 overview，只接受
   更高 revision；写动作一律 pending → 回执，没有乐观成功；下一动作只取自 Core
   的错误分类，不解释原始异常字符串，也不展示或推断 scheduler 状态。
   数据生命周期只在类别激活时挂载，未激活时不发起任何 IPC。 */
function PersonalContextPane ({ shell }: { shell: Dict }): ReactElement {
  const copy = PC.PERSONAL_CONTEXT_COPY
  const [status, setStatus] = useState<'loading' | 'ready' | 'unavailable'>('loading')
  const [failure, setFailure] = useState<Dict | null>(null)
  const [snapshot, setSnapshot] = useState<Dict | null>(null)
  const [page, setPage] = useState<Dict | null>(null)
  const [resource, setResource] = useState<PersonalContextResource>('session_episodes')
  const [attempt, setAttempt] = useState(0)
  const [expanded, setExpanded] = useState<string | null>(null)
  const [draft, setDraft] = useState<Dict | null>(null)
  const [draftProblem, setDraftProblem] = useState('')
  const [entry, setEntry] = useState<Dict>({ open: false, text: '', kind: 'preference' })
  const [entryProblem, setEntryProblem] = useState('')
  const [confirm, setConfirm] = useState<Dict | null>(null)
  const [pending, setPending] = useState('')
  const [notice, setNotice] = useState('')
  const [writeFailure, setWriteFailure] = useState<Dict | null>(null)
  const applied = useRef<number | null>(null)
  const generation = useRef(0)
  const activeResource = useRef<PersonalContextResource>('session_episodes')
  const nextRequestId = useRef(PC.createRequestIds('settings.context'))
  const confirmPanel = useRef<HTMLDivElement>(null)
  const confirmTrigger = useRef<HTMLElement | null>(null)

  /* quiet 用于「已有权威内容、只是再读一遍」的场景（changed 事件、写回执之后）：
     保留上一份权威投影直到新投影到达，避免每次保存都把列表闪成骨架。它不改变
     收束方式 —— 成功仍整页替换，失败仍整类转不可用，界面不因此宣称写入成功。 */
  const load = useCallback(async (target: PersonalContextResource, quiet = false) => {
    const token = generation.current + 1
    generation.current = token
    const stale = (): boolean => token !== generation.current
    const stop = (described: Dict) => { if (!stale()) { setStatus('unavailable'); setFailure(described) } }
    setFailure(null)
    if (!quiet) { setStatus('loading'); setPage(null) }
    let overview: Dict
    try { overview = await shell.getAgentContextOverview(PC.overviewRequest()) }
    catch { stop({ message: copy.unavailable, action: 'retry' }); return }
    if (stale()) return
    if (overview?.ok !== true || overview.snapshot == null) { stop(PC.describeFailure(overview?.error)); return }
    applied.current = overview.snapshot.revision
    setSnapshot(overview.snapshot)
    let listing: Dict
    try { listing = await shell.manageAgentContext(PC.viewRequest(nextRequestId.current('view'), target)) }
    catch { stop({ message: copy.unavailable, action: 'retry' }); return }
    if (stale()) return
    if (listing?.ok !== true || listing.result == null) { stop(PC.describeFailure(listing?.error)); return }
    if (PC.isHigherRevision(applied.current, listing.revision)) applied.current = listing.revision
    setPage(listing.result); setStatus('ready')
  }, [copy.unavailable, shell])

  /* 重试与重新载入走同一条路径：重新订阅、重新读取权威 snapshot。任何本地成功态
     都不保留，订阅缺失时也不会只靠读取继续渲染。 */
  const reload = useCallback(() => setAttempt((value) => value + 1), [])

  useEffect(() => {
    let unsubscribe: unknown = null
    try {
      unsubscribe = shell.onAgentContextChanged((event: Dict) => {
        if (!PC.isHigherRevision(applied.current, event?.revision)) return
        void load(activeResource.current, true)
      })
    } catch {
      setStatus('unavailable'); setFailure({ message: copy.unavailable, action: 'retry' })
      return
    }
    void load(activeResource.current)
    return () => { generation.current += 1; if (typeof unsubscribe === 'function') unsubscribe() }
  }, [attempt, copy.unavailable, load, shell])
  useEffect(() => { if (confirm !== null) confirmPanel.current?.focus() }, [confirm])

  const submit = useCallback(async (key: string, request: Dict, onReceipt: (response: Dict) => void) => {
    setPending(key); setWriteFailure(null); setNotice(copy.pending)
    let response: Dict
    try { response = await shell.manageAgentContext(request) }
    catch {
      setPending(''); setNotice('')
      setWriteFailure({ message: copy.unsettled, action: 'reload', preserveEdits: false })
      reload(); return
    }
    setPending('')
    if (response?.ok === true && response.result != null) {
      if (PC.isHigherRevision(applied.current, response.revision)) applied.current = response.revision
      onReceipt(response)
      void load(activeResource.current, true)
      return
    }
    setNotice('')
    const described = PC.describeFailure(response?.error)
    setWriteFailure(described)
    if (described.action === 'reload' && described.preserveEdits !== true) reload()
  }, [copy.pending, copy.unsettled, load, reload, shell])

  const chooseResource = (next: PersonalContextResource) => {
    if (next === resource) return
    activeResource.current = next; setResource(next)
    setExpanded(null); setDraft(null); setDraftProblem(''); setWriteFailure(null); setNotice('')
    void load(next)
  }
  const openConfirm = (next: Dict, event: { currentTarget: HTMLElement }) => {
    confirmTrigger.current = event.currentTarget
    setConfirm(next)
  }
  const closeConfirm = () => {
    setConfirm(null)
    const trigger = confirmTrigger.current; confirmTrigger.current = null
    trigger?.focus()
  }
  const acceptConfirm = () => {
    const target = confirm; const expected = snapshot?.revision
    closeConfirm()
    if (target === null || typeof expected !== 'number') return
    if (target.action === 'forget') {
      void submit(`forget:${target.item.memory_id}`,
        PC.forgetRequest(nextRequestId.current('forget'), expected, target.item), () => setNotice(copy.forgetSaved))
      return
    }
    if (target.action === 'delete') {
      void submit(`delete:${target.item.memory_id}`,
        PC.deleteRequest(nextRequestId.current('delete'), expected, target.item),
        (response) => setNotice(PC.describeDeletion(response.result)))
      return
    }
    void submit('processing', PC.setProcessingRequest(nextRequestId.current('set-processing'), expected, target.state),
      (response) => setNotice(PC.describeProcessingResult(response.result.memory_processing)))
  }
  const submitRemember = () => {
    const problem = PC.describeEntryProblem(entry.text)
    setEntryProblem(problem)
    const expected = snapshot?.revision
    if (problem !== '' || typeof expected !== 'number') return
    const structured = PC.structuredEntry({
      displayText: entry.text, kind: entry.kind, scopeKind: PC.REMEMBER_SCOPE_KIND, scopeReference: null
    })
    void submit('remember', PC.rememberRequest(nextRequestId.current('remember'), expected, structured), () => {
      setEntry({ open: false, text: '', kind: 'preference' }); setNotice(copy.rememberSaved)
    })
  }
  const submitUpdate = (item: Dict) => {
    const text = String(draft?.text ?? '')
    const problem = PC.describeEntryProblem(text)
    setDraftProblem(problem)
    const expected = snapshot?.revision
    if (problem !== '' || typeof expected !== 'number') return
    const structured = PC.structuredEntry({
      displayText: text, kind: item.kind, scopeKind: item.scope.kind, scopeReference: item.scope.reference
    })
    void submit(`update:${item.memory_id}`, PC.updateRequest(nextRequestId.current('update'), expected, item, structured),
      () => { setDraft(null); setNotice(copy.updateSaved) })
  }

  if (status !== 'ready') {
    return <div className="pc" data-status={status}>{status === 'loading'
      ? <div className="group pc-loading" id="pcLoading" aria-busy={true}>
          <p className="hint">{copy.loading}</p><span className="pc-skeleton" aria-hidden="true" /></div>
      : <div className="group pc-unavailable" id="pcUnavailable"><p role="alert">{failure?.message ?? copy.unavailable}</p>
          <button className="secondary-btn" id="pcRetry" onClick={reload}>
            {failure?.action === 'reload' ? copy.reloadAction : copy.retryAction}</button></div>}</div>
  }

  const processing: Dict = snapshot?.memory_processing ?? {}
  const nextProcessingState = processing.state === 'enabled' ? 'suspended' : 'enabled'
  const processingPending = pending === 'processing'
  const counts: Dict | null = snapshot?.counts ?? null
  const items: Dict[] = page?.items ?? []
  const confirmTitle = confirm === null ? ''
    : confirm.action === 'forget' ? copy.forgetAction
      : confirm.action === 'delete' ? copy.deleteAction : copy.processingLabel
  const confirmBody = confirm === null ? ''
    : confirm.action === 'forget' ? copy.forgetConfirm
      : confirm.action === 'delete' ? copy.deleteConfirm
        : confirm.state === 'suspended' ? copy.suspendConfirm : copy.resumeConfirm

  return <div className="pc" data-status="ready">
    <div className="group pc-processing"><div className="row">
      <div><div className="label">{copy.processingLabel}</div>
        <div className="hint" id="pcProcessingState">{PC.PROCESSING_STATE_LABELS[processing.state] ?? ''}</div>
        <div className="hint" id="pcProcessingBoundary">{PC.PROCESSING_BOUNDARY_LABELS[processing.automatic_processing_boundary] ?? ''}</div></div>
      <label className="switch"><input id="pcProcessingToggle" type="checkbox" checked={processing.state === 'enabled'}
        disabled={processingPending} aria-busy={processingPending} aria-describedby="pcProcessingState"
        onChange={(event) => openConfirm({ action: 'processing', state: nextProcessingState, item: null }, event)} />
        <span>{copy.processingSwitch}</span></label>
    </div></div>

    <div className="group pc-sections">
      <div className="seg pc-tabs" id="pcSections" role="radiogroup" aria-label="事实分区">
        {([['session_episodes', copy.episodesTab], ['personal_memories', copy.memoriesTab]] as ReadonlyArray<readonly [PersonalContextResource, string]>)
          .map(([key, label]) => <button key={key} role="radio" data-resource={key} aria-checked={resource === key}
            className={resource === key ? 'on' : ''} onClick={() => chooseResource(key)}>{label}
            {counts === null ? '' : `（${key === 'personal_memories' ? counts.personal_memories : counts.session_episodes}）`}</button>)}
      </div>
      <p className="hint" id="pcSectionHint">{resource === 'personal_memories' ? copy.memoriesHint : copy.episodesHint}</p>
    </div>

    <p className="pc-status" id="pcStatus" role="status" aria-live="polite">{notice}</p>
    <div className="pc-alert" id="pcAlert" role="alert" hidden={writeFailure === null}>
      <span>{writeFailure?.message ?? ''}</span>
      {writeFailure !== null && (writeFailure.action === 'reload' || writeFailure.action === 'retry')
        ? <button className="link-btn" id="pcReload" onClick={reload}>
            {writeFailure.action === 'reload' ? copy.reloadAction : copy.retryAction}</button>
        : null}
    </div>

    {resource === 'personal_memories'
      ? <div className="group pc-list-group">
          {items.length === 0
            ? <p className="pc-empty" id="pcMemoriesEmpty">{copy.memoriesEmpty}</p>
            : <ul className="pc-list" id="pcMemories" aria-label={copy.memoriesTab}>{items.map((item) => {
                const open = expanded === item.memory_id
                const editing = draft?.memoryId === item.memory_id
                const rowPending = pending.endsWith(`:${item.memory_id}`)
                return <li key={item.memory_id} className="pc-item" data-memory-id={item.memory_id} data-lifecycle={item.lifecycle}>
                  <button className="pc-row" aria-expanded={open} aria-label={PC.memoryRowName(item)} aria-busy={rowPending}
                    onClick={() => setExpanded(open ? null : item.memory_id)}>
                    <span className="pc-text">{item.display_text}</span>
                    <span className="pc-meta"><span>{PC.memoryScopeText(item)}</span><span>{PC.MEMORY_KIND_LABELS[item.kind] ?? ''}</span>
                      <span>{PC.ORIGIN_LABELS[item.origin] ?? ''}</span>
                      <span data-field="lifecycle">{PC.LIFECYCLE_LABELS[item.lifecycle] ?? ''}</span></span>
                  </button>
                  {open ? <div className="pc-detail">
                    <p className="hint">{PC.describeSourceReferences(item.source_reference_count)} · {PC.describeUpdatedAt(item.updated_at)}</p>
                    {editing
                      ? <div className="pc-form">
                          <label htmlFor="pcDraftText">内容</label>
                          <textarea id="pcDraftText" value={String(draft?.text ?? '')}
                            onChange={(event) => setDraft({ memoryId: item.memory_id, text: event.currentTarget.value })} />
                          <p className="pc-problem" role="alert" hidden={draftProblem === ''}>{draftProblem}</p>
                          <div className="pc-actions">
                            <button className="primary-btn" data-action="save" disabled={rowPending} aria-busy={rowPending}
                              onClick={() => submitUpdate(item)}>{copy.saveUpdate}</button>
                            <button className="link-btn" data-action="cancel"
                              onClick={() => { setDraft(null); setDraftProblem('') }}>{copy.cancelEdit}</button></div>
                        </div>
                      : <div className="pc-actions">
                          <button className="secondary-btn" data-action="edit" aria-label={PC.memoryActionName('editAction', item)}
                            onClick={() => { setDraft({ memoryId: item.memory_id, text: item.display_text }); setDraftProblem('') }}>{copy.editAction}</button>
                          <button className="secondary-btn" data-action="forget" disabled={rowPending} aria-busy={rowPending}
                            aria-label={PC.memoryActionName('forgetAction', item)}
                            onClick={(event) => openConfirm({ action: 'forget', item }, event)}>{copy.forgetAction}</button>
                          <button className="secondary-btn" data-action="delete" disabled={rowPending} aria-busy={rowPending}
                            aria-label={PC.memoryActionName('deleteAction', item)}
                            onClick={(event) => openConfirm({ action: 'delete', item }, event)}>{copy.deleteAction}</button></div>}
                  </div> : null}
                </li>
              })}</ul>}
          {page?.has_more === true ? <p className="hint pc-more" id="pcMoreMemories">{copy.moreRecords}</p> : null}
        </div>
      : <div className="group pc-list-group">
          {items.length === 0
            ? <p className="pc-empty" id="pcEpisodesEmpty">{copy.episodesEmpty}</p>
            : <ul className="pc-list" id="pcEpisodes" aria-label={copy.episodesTab}>{items.map((episode) => {
                const open = expanded === episode.episode_id
                return <li key={episode.episode_id} className="pc-item" data-episode-id={episode.episode_id}>
                  <button className="pc-row" aria-expanded={open} aria-label={PC.episodeRowName(episode)}
                    onClick={() => setExpanded(open ? null : episode.episode_id)}>
                    <span className="pc-text">{episode.summary.title}</span>
                    <span className="pc-meta"><span>{PC.SOURCE_KIND_LABELS[episode.source_kind] ?? ''}</span>
                      <span>{PC.memoryScopeText(episode)}</span>
                      <span>{PC.describeRelativeRange(episode.occurred_from_offset_ms, episode.occurred_through_offset_ms)}</span></span>
                  </button>
                  {open ? <div className="pc-detail">
                    <ul className="pc-bullets">{episode.summary.bullets.map((bullet: string, index: number) => <li key={index}>{bullet}</li>)}</ul>
                    {episode.omissions.map((omission: string) => <p className="pc-omission" key={omission} data-omission={omission}>
                      {PC.OMISSION_LABELS[omission] ?? ''}</p>)}
                    <p className="hint">{PC.describeSourceReferences(episode.source_reference_count)} · {PC.describeUpdatedAt(episode.updated_at)}</p>
                  </div> : null}
                </li>
              })}</ul>}
          {page?.has_more === true ? <p className="hint pc-more" id="pcMoreEpisodes">{copy.moreRecords}</p> : null}
        </div>}

    {resource === 'personal_memories'
      ? <div className="group pc-remember"><div className="row">
          <div><div className="label">{copy.rememberAction}</div><div className="hint">{copy.rememberScopeNote}</div></div>
          <button className="secondary-btn" id="pcRememberOpen" aria-expanded={entry.open === true}
            onClick={() => { setEntry({ ...entry, open: entry.open !== true }); setEntryProblem('') }}>
            {entry.open === true ? copy.dismissAction : copy.rememberAction}</button></div>
          {entry.open === true ? <div className="pc-form">
            <label htmlFor="pcRememberText">内容</label>
            <textarea id="pcRememberText" value={String(entry.text)}
              onChange={(event) => setEntry({ ...entry, text: event.currentTarget.value })} />
            <label htmlFor="pcRememberKind">类型</label>
            <select id="pcRememberKind" value={String(entry.kind)}
              onChange={(event) => setEntry({ ...entry, kind: event.currentTarget.value })}>
              {Object.keys(PC.MEMORY_KIND_LABELS).map((kind) => <option key={kind} value={kind}>{PC.MEMORY_KIND_LABELS[kind]}</option>)}</select>
            <p className="hint">范围：{PC.SCOPE_KIND_LABELS[PC.REMEMBER_SCOPE_KIND]}</p>
            <p className="pc-problem" role="alert" hidden={entryProblem === ''}>{entryProblem}</p>
            <button className="primary-btn" id="pcRememberSubmit" disabled={pending === 'remember'}
              aria-busy={pending === 'remember'} onClick={submitRemember}>{copy.rememberAction}</button>
          </div> : null}
        </div>
      : null}

    {confirm !== null
      ? <div className="pc-confirm" id="pcConfirm" role="dialog" aria-labelledby="pcConfirmTitle" aria-describedby="pcConfirmBody"
          tabIndex={-1} ref={confirmPanel}
          onKeyDown={(event) => { if (event.key === 'Escape') { event.stopPropagation(); closeConfirm() } }}>
          <h2 id="pcConfirmTitle">{confirmTitle}</h2>
          <p id="pcConfirmBody">{confirmBody}</p>
          {confirm.item == null ? null : <p className="pc-confirm-target">{confirm.item.display_text}</p>}
          <div className="pc-actions">
            <button className="primary-btn" id="pcConfirmAccept" onClick={acceptConfirm}>{copy.confirmAction}</button>
            <button className="secondary-btn" id="pcConfirmDismiss" onClick={closeConfirm}>{copy.dismissAction}</button></div>
        </div>
      : null}
  </div>
}
  ...
  return <>
    ...
    <div className="layout"><nav className="nav" aria-label="设置类别">{PANES.map(([name, label]) => <button key={name} className={`nav-item${pane === name ? ' active' : ''}`} data-pane={name} aria-current={pane === name ? 'page' : undefined} onClick={() => setPane(name)}>{label}</button>)}</nav>
      <main className="content">
        ...
        <section className={`pane${pane === 'context' ? ' active' : ''}`} data-pane="context"><h1>{PC.PERSONAL_CONTEXT_COPY.category}</h1><p className="sub">{PC.PERSONAL_CONTEXT_COPY.categorySub}</p>
          {pane === 'context' ? <PersonalContextPane shell={shell} /> : null}</section>
