...
function App () {
  ...
  return <div className="app-shell">
    ...
    <section className="provider-strip" aria-label="Agent 模型 provider">
      ...
      <label>类型<select data-testid="provider-type" value={form.provider} onChange={(event) => setForm({ ...form, provider: event.target.value, model: event.target.value === 'deterministic-test' ? 'fixture-model' : form.model })}>
      ...
      {form.provider === 'openai-compatible' && <>
        <label>HTTPS 地址<input data-testid="provider-url" value={form.baseUrl} onChange={(event) => setForm({ ...form, baseUrl: event.target.value })} placeholder="https://example.com/v1" /></label>
        <label>模型<input data-testid="provider-model" value={form.model} onChange={(event) => setForm({ ...form, model: event.target.value })} /></label>
        <label>新凭据<input data-testid="provider-key" type="password" value={form.apiKey} onChange={(event) => setForm({ ...form, apiKey: event.target.value })} placeholder={state?.provider.hasCredential ? '已配置；留空保持不变' : '仅提交到主进程'} /></label>
        <label className="consent"><input data-testid="provider-disclosure" type="checkbox" checked={form.cloudDisclosureAccepted} onChange={(event) => setForm({ ...form, cloudDisclosureAccepted: event.target.checked })} />我确认终态会话正文将发送给此服务</label>
      </>}
      <button data-testid="provider-save" onClick={saveProvider} disabled={busy !== null}>保存</button>
      <span data-testid="credential-state" className="credential-state">{state?.provider.hasCredential ? (state.provider.credentialPersisted ? '凭据已加密保存在本机' : '凭据仅本次进程可用') : form.provider === 'deterministic-test' ? '无需凭据' : '尚未配置凭据'}</span>
    </section>

    {error && <div className="error" role="alert" data-error-code={error}>{errorText[error] || error}</div>}

    <main className="workspace">
      <aside className="sessions">
        <div className="panel-heading"><div><strong>终态会话</strong><span>仅合成、无音频</span></div></div>
        <div className="fixture-actions"><button data-testid="fixture-loopback" onClick={() => createFixture('loopback')} disabled={busy !== null}>添加系统音频 fixture</button><button data-testid="fixture-mic" onClick={() => createFixture('mic')} disabled={busy !== null}>添加麦克风 fixture</button></div>
        <div className="session-list">{state?.runtime.sessions.map((session) => <button data-testid="session-item" key={session.sessionId} className={selected === session.sessionId ? 'selected' : ''} onClick={() => setSelected(session.sessionId)}>
          <strong>{session.sourceId === 'loopback' ? '系统音频' : '麦克风'}</strong><span>{shortId(session.sessionId)}</span><small>{session.state}</small>
        </button>)}</div>
      </aside>

      <section className="conversation">
        <div className="panel-heading"><div><strong>调试聊天</strong><span>{selectedSession ? `${selectedSession.sourceId === 'loopback' ? '系统音频' : '麦克风'} · ${shortId(selectedSession.sessionId)}` : '请先选择终态会话'}</span></div><button data-testid="preview-reference" onClick={makePreview} disabled={!selected || busy !== null}>预览参考产物</button></div>
        <div className="message-list">{messages.length === 0 && <p className="empty">聊天记录单独保存在隔离 SQLite，不进入个人记忆。</p>}{messages.map((message) => <article data-testid="message-item" key={message.messageId} className={`message ${message.role}`}>
        ...
        <div className="composer"><textarea data-testid="chat-prompt" value={prompt} onChange={(event) => setPrompt(event.target.value)} disabled={!selected || busy !== null} /><button data-testid="chat-send" onClick={sendChat} disabled={!selected || !prompt.trim() || busy !== null}>{busy === 'chat' ? '响应中…' : '发送'}</button></div>
      </section>

      <aside className="inspector">
        <div className="panel-heading"><div><strong>上下文与任务</strong><span>只显示脱敏身份与状态</span></div></div>
        {preview && <section data-testid="preview-panel" className="preview"><strong>执行预览</strong><dl><dt>固定 recipe</dt><dd>{preview.recipeId}</dd><dt>输入水位</dt><dd>{preview.inputRef.inputWatermark}</dd><dt>输入 digest</dt><dd>{shortId(preview.inputRef.inputDigest)}</dd><dt>云端正文披露</dt><dd>{preview.cloudDisclosure ? '是' : '否'}</dd></dl><div><button data-testid="preview-accept" onClick={() => decide('accepted')}>确认执行</button><button data-testid="preview-reject" className="secondary" onClick={() => decide('rejected')}>拒绝</button></div></section>}
        <section><h2>后台任务</h2>{state?.runtime.jobs.length === 0 && <p className="empty">暂无任务</p>}{state?.runtime.jobs.map((job) => <div data-testid="job-item" data-state={job.state} className="job" key={job.runId}><div><strong>{job.state}</strong><span>{shortId(job.runId)}</span></div><small>尝试 {job.attemptCount} · {job.model}</small>{job.errorCode && <em>{job.errorCode}</em>}{['queued', 'retry_wait', 'running'].includes(job.state) && <button onClick={() => act('cancel', () => window.agentMvp.cancel(job.runId))}>取消</button>}</div>)}</section>
        <section><h2>参考结构化产物</h2>{state?.runtime.artifacts.length === 0 && <p className="empty">暂无产物</p>}{state?.runtime.artifacts.map((artifact) => <article data-testid="artifact-item" className="artifact" key={artifact.artifactId}><strong>{artifact.content.title}</strong><small>{artifact.type} · {shortId(artifact.runId)}</small><ul>{artifact.content.bullets.map((bullet) => <li key={bullet}>{bullet}</li>)}</ul></article>)}</section>
        <section><h2>受控工具事件</h2>{events.slice(-6).map((entry, index) => <div data-testid="tool-event" className="event" key={`${entry.runId}-${index}`}><span>{entry.event.type}</span>{entry.event.toolName && <small>{entry.event.toolName}</small>}</div>)}</section>
      </aside>
    </main>
  </div>
}
