...
function validateInteractionEvidence (bytes, expectedScenario = null) {
  ...
  if ([5, 6].includes(report.schemaVersion) && report.scenario === 'dwm-drag') {
...
function validateDwmCompanion (report, completionBytes) {
  if (![3, 4, 5, 6].includes(report.schemaVersion) || report.scenario !== 'dwm-drag') return null
...
if (require.main === module) {
  ...
  if ([3, 4, 5, 6].includes(report.schemaVersion) && report.scenario === 'dwm-drag') {
    ...
    throw new Error('--completion is only valid for schema-v3/schema-v4/schema-v5/schema-v6 dwm-drag reports')
  }
