...
> 更新日期：2026-08-10
> 证据状态：SEM-F29/J23 隔离 Agent 内核开发入口为联合验收完成；D3 的正式 SQLite migration 与存储/生命周期子边界、D4 的会后结构化纪要后端纵切、D5 的增强文本与个人记忆 UI-free 后端纵切、D6 的正式 storage utility transport 子边界、D9 的无公网 main-only Agent 模型 provider bootstrap/catalog 子边界，以及 D10 的 `AgentModelProviderRegistry → ModelGateway → Pi Agent Loop` 无公网内部路径均为实现完成·尚未验收；ADR 0011 的正式 main、Agent utility 与网络接线仍为已决定，其余正式产品项保持已决定。
