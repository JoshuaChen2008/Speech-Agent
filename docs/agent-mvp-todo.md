...
> 更新日期：2026-08-10
> 证据状态：SEM-F29/J23 隔离 Agent 内核开发入口为联合验收完成；D3 的正式 SQLite migration 与存储/生命周期子边界、D4 的会后结构化纪要后端纵切、D5 的增强文本与个人记忆 UI-free 后端纵切、D6 的正式 storage utility transport 子边界、D9 的无公网 main-only Agent 模型 provider bootstrap/catalog 子边界、D10 的 `AgentModelProviderRegistry → ModelGateway → Pi Agent Loop` 无公网内部路径、D11 的 `ConfigStore` v2 Agent 设置存储，以及 D12 的字幕提交边界接线子边界均为实现完成·尚未验收；D13 正式任务存储网关接线与 ADR 0011 的正式 main、Agent utility、网络接线仍为已决定，其余正式产品项保持已决定。
...
| 增强文本与个人记忆后端纵切 | 实现完成·尚未验收 | 正式 `enhanced-transcript`、`memory-extraction`、任务内 `memory-consolidation`、writer 分流、三项同输入独立执行、记忆三级筛选/去重/冲突/suppression 与单条删除已有真实 storage worker/SQLite/PluginHost/job runner 组合证据；D8 又为受信任策略门控、休眠/恢复、active/current revision 投影和双重预算形成 `MemoryReader → StorageWorkerService/FormalAgentStore` 子边界证据；不包含 `MeetingStopped`、正式 `StorageGateway`、Agent utility、preload/IPC、renderer 或确认关键词 |
...
| 正式字幕提交边界接线 | 实现完成·尚未验收 | D12 已形成 UI-free `ConfigStore → FormalAgentRuntime → SessionCoordinator → MeetingStoppedPersistenceSink → SqliteSessionRecorder → StorageGateway → storage utility/SQLite` 纵切，并以既有 D6 旅程覆盖持久 close 后尽力通知、下一会话先开始、exact-child replacement、当前代次策略先行和显式恢复；不包含正式 main/preload/renderer、Agent utility、Agent job runner 完整 `StorageGateway` 路由或真实 DeepSeek HTTP |
| 正式三项后台 Agent 任务 | 已决定 | 三项任务共享同一冻结输入并独立执行的 UI-free 后端子边界为实现完成·尚未验收；D13 已登记将 runner、冻结字幕/个人记忆读取与两类提交统一接入正式 `StorageGateway`，但 Agent utility、用户读取/操作链路与完整 J21/J24 仍无产品证据 |
...
| D13 | `AgentJobRunner`、`TranscriptReader`、`MemoryReader` 与两类 writer 统一接入 main-owned `StorageGateway`；在模型结果返回后、提交前强制收殓 exact storage child，以同一身份恢复并有界读回个人记忆；升级 D6/D12 既有旅程，不新增同义测试 | 待 Luna/max 独立复核 | 待实现后验证 | — | 已决定 |
