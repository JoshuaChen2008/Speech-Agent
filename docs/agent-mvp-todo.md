...
> 更新日期：2026-08-10
> 证据状态：SEM-F29/J23 隔离 Agent 内核开发入口为联合验收完成；D3 的正式 SQLite migration 与存储/生命周期子边界、D4 的会后结构化纪要后端纵切、D5 的增强文本与个人记忆 UI-free 后端纵切、D6 的正式 storage utility transport 子边界、D9 的无公网 main-only Agent 模型 provider bootstrap/catalog 子边界，以及 D10 的 `AgentModelProviderRegistry → ModelGateway → Pi Agent Loop` 无公网内部路径均为实现完成·尚未验收；D11 的 `ConfigStore` v2 Agent 设置切片为已决定；ADR 0011 的正式 main、Agent utility 与网络接线仍为已决定，其余正式产品项保持已决定。
...
| 正式 Agent 设置存储 | 已决定 | D11 已登记平面 `ConfigStore` v1→v2、六个 Agent 设置事实、revision 原子更新及 Agent/个人记忆自动处理边界；待实现真实 ConfigStore→storage worker/SQLite 联合旅程，不包含 provider 参数、API key、正式 IPC/preload/renderer 或完整 J24 |
...
| D11 | `ConfigStore` v1→v2 Agent 设置事实、revision 冲突、Agent/个人记忆自动处理边界及真实 storage worker/SQLite 资格组合；provider 配置与凭据继续由 D9 main-only bootstrap 管理 | 待 Luna/max 独立复核 | 待实现后验证 | — | 已决定 |
