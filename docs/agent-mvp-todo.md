...
> 更新日期：2026-08-10
> 证据状态：SEM-F29/J23 隔离 Agent 内核开发入口为联合验收完成；D3 的正式 SQLite migration 与存储/生命周期子边界、D4 的会后结构化纪要后端纵切、D5 的增强文本与个人记忆 UI-free 后端纵切、D6 的正式 storage utility transport 子边界、D9 的无公网 main-only Agent 模型 provider bootstrap/catalog 子边界、D10 的 `AgentModelProviderRegistry → ModelGateway → Pi Agent Loop` 无公网内部路径，以及 D11 的 `ConfigStore` v2 Agent 设置存储子边界均为实现完成·尚未验收；ADR 0011 的正式 main、Agent utility 与网络接线仍为已决定，其余正式产品项保持已决定。
...
| 正式字幕提交边界接线 | 已决定 | D12 已登记 UI-free `SessionCoordinator → MeetingStoppedPersistenceSink → FormalAgentRuntime/AgentJobReconciler → StorageGateway` 纵切，待升级 D6 正式 utility-process 旅程；不修改冲突区中的正式 main/preload/renderer |
...
| D12 | main-owned `FormalAgentRuntime`、exact 资格上下文、设置/任务策略串行化，以及持久 close 后 detached `MeetingStopped` 对账；升级 D6 真实 `SessionCoordinator/SqliteSessionRecorder/StorageGateway/storage utility/SQLite` 旅程，不新增同义测试 | 待 Luna/max 独立复核 | 待实现后验证 | — | 已决定 |
