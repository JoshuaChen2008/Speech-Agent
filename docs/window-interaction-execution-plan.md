...
> 本文只规定后续实施顺序与验收边界；当前变更不修改产品代码、测试代码或证据 schema。
...
3. 工具条无论锁定状态都只由明确可见的握把发起拖动。解锁时握把移动字幕与工具条组合；锁定时字幕窗恒穿透，握把只移动工具条。
...
| `src/toolbar/toolbar.js` | `.act` 之外的整个工具条都能开始拖动；现有 `.grip` 在解锁时隐藏 | 复用并持续显示现有握把，只有握把能开始拖动，状态文字和控件间隙不承担隐藏拖动 |
...
| 测试 / 实机证据 | 尚无 J17 证据；既有 I2 `dwm-drag` 仅为 schema-v1 场景，尚无符合 SEM-F22 的逐项实机证据 | 先取得确定性联合旅程，再取得真实 DWM 与 DPI/异缩放观察 |
...
- `test/integration/window-interaction-journey.test.js`
  ...
  - 串起首帧 fail-safe → 有效矩形收缩 → 状态宽度变化 → reload generation 失效 → 新矩形恢复 → 字幕拖动/拉伸/原地点击 → 解锁/锁定握把 → 设置/字幕历史焦点往返。
...
5. 复用现有 `.grip`，改成两种锁定状态下始终可见的中性点阵握把，交互目标至少 `24 × 30 CSS px`；保持非按钮、不可聚焦和 `aria-hidden`。只有握把监听 `pointerdown`。解锁时沿现有主进程分支移动组合，锁定时只移动 toolbarWin。
...
- product-shell 报告升级到 schema v5；`scripts/verify-product-shell-report.js` 只接受相应布尔结果、计数与源码哈希，报告不得写矩形、桌面坐标、窗口位置、字幕正文、本地绝对路径、设备名或绝对单调时刻。
...
4. 解锁与锁定状态都只有握把能拖工具条；锁定时字幕窗保持穿透。
...
`dwm-drag` 升级到 schema v3，继续兼容读取既有 v1/v2 报告。DWM harness 必须复用生产窗口交互控制器、preload 和页面资源，不复制简化版拖动实现。报告只增加闭集场景枚举、布尔、非负计数与哈希，不得包含工具条矩形、屏幕坐标、窗口绝对位置、字幕正文、本地绝对路径、设备名、绝对单调时刻或时钟偏移。严格 verifier 必须拒绝未知字段、漏项、重复组合、越界计数和只声明 completion 而未逐项确认的报告。
...
| 工具条握把 | 解锁移动组合；锁定只移动工具条 | 握把可发现且行为一致 |
