...
- D12 的 `MeetingStoppedPersistenceSink` 位于 `SessionCoordinator` 与现有 `SqliteSessionRecorder` 之间，但不改变字幕 persistence sink 合同：只有终态 close 取得持久确认后才 detached 通知 main-owned `FormalAgentRuntime`，`stop` 不等待对账且下一会话可以先开始。runtime 从 ConfigStore v2 与 D9 bootstrap 组合 exact 非敏感资格上下文；`StorageGateway` 把任务策略确认绑定到当前 active storage worker 代次，replacement、关闭或强制终止都清除旧确认。启动或 replacement 恢复必须先让当前代次重放任务策略，再对调用方分页提供的终态 `sessionId` 补建；未确认策略时对账 fail closed。通知失败只保留稳定诊断并等待后续恢复，不把 Agent 失败改写为字幕停止失败。
- D13 已把 `AgentJobRunner`、冻结字幕读取、个人记忆读取与两类 writer 全部收口到同一个 main-owned `StorageGateway`，不再把具体 storage worker 代次交给任务运行时。模型结果返回后、原子提交前的 storage child replacement 保持同一 `runId`、lease 与提交身份；当前代次未重放任务策略时，真实 runner 入口暂停领取。该 UI-free 接线子边界为实现完成·尚未验收，不表示 Agent Loop 已进入 Agent utility。
- D14 已把正式 `AgentPluginHost`、输入规划、provider registry、`ModelGateway` 与 Pi Agent Loop 移入独立 Agent utility；main 保留 `AgentJobRunner`、`TranscriptReader`、租约和两个 writer。main proxy 只发送 exact claimed job、已由 `StorageGateway` 验证的冻结输入、受信任 provider 配置和一次性凭据字节，utility 不取得 storage port，只返回 exact `PluginResult`。child 使用 D9 净化环境；未知响应、timeout 或异常退出会同步失效 generation、拒绝 pending、忽略迟到成功并等待 exact child 退出，main 凭据随即失效且当前进程停止领取。只有新的 Electron main 启动从新的启动环境组合凭据并重放策略后，才能创建新 generation 并恢复同一 `runId`。utility 的调用级 bootstrap 不读环境、不发布资格或重建配置表；环境若出现 provider 凭据则先清零本次调用副本再异常退出。该 UI-free 进程接线子边界为实现完成·尚未验收，不创建网络适配器或正式窗口。
- D15 按 ADR 0012 登记 main-owned `FormalAgentJobScheduler` 与字幕会话资源桥接：一个 logical claim attempt 持有同一 claim receipt 请求身份（含请求 `leaseMs`，不含 receipt 返回的任务 lease）直至确定收束；单 owner `wakeEpoch` 在 idle 时只排一个 drain，并在空扫描转 idle 前复核，避免用轮询掩盖丢唤醒；不可重启的 `stop` 以 generation 清除 timer/wake。桥接执行 subscribe-first + revision 单调复读，只以 `SessionCoordinator` 公开快照中的 `sessionId` 判定活动字幕会话，并把结果冻结到下一 attempt 的 `localWorkAllowed`；停止桥接时取消订阅并忽略旧 generation 回调。云端任务继续运行且字幕会话不等待调度器；exact `{ code: 'AGENT_SCHEDULER_FAILED' }` 只属 observer 诊断，不进入任务错误码。运行中本地任务有界停止、跨进程 claim 原子性与正式 main/preload/renderer 均未由 D15 闭合。
