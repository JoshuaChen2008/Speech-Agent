...
> 当前实现投影（2026-08-10）：D3–D6 的正式存储、三项 UI-free 后端纵切与 production `StorageWorkerHost` utility-process transport，D8 的 `MemoryReader → StorageWorkerService/FormalAgentStore` 有界读取，以及 D9–D13 的 main-only provider bootstrap/catalog、registry/Gateway/Pi 内部路径、ConfigStore v2 Agent 设置、UI-free `MeetingStopped`/`AgentJobReconciler` 和正式任务 `StorageGateway` 接线子边界，均为实现完成·尚未验收。D13 已让真实 runner、冻结字幕读取、个人记忆读取与两类 writer 共享 main-owned `StorageGateway`，并覆盖提交前 exact-child replacement 与当前代次策略先行。PluginHost/Loop 仍未进入 Agent utility，正式 main/preload/renderer、真实 DeepSeek HTTP、确认关键词、资源仲裁与正式打包仍为已决定。
...
- D14 登记把正式 `AgentPluginHost`、输入规划、provider registry、`ModelGateway` 与 Pi Agent Loop 移入独立 Agent utility；main 保留 `AgentJobRunner`、`TranscriptReader`、租约和两个 writer。main proxy 只发送 exact claimed job、已由 `StorageGateway` 验证的冻结输入、受信任 provider 配置和一次性凭据字节，utility 不取得 storage port，只返回 exact `PluginResult`。child 使用 D9 净化环境；异常退出后 main 凭据失效、当前进程停止领取，只有新的 Electron main 启动从新的启动环境组合凭据并重放策略后，才能创建新 generation 并恢复同一 `runId`。utility 的调用级 bootstrap 不读环境、不发布资格或重建配置表。该 UI-free 进程接线不创建网络适配器或正式窗口。
