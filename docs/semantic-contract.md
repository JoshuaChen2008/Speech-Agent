...
| **SEM-F04** | 一遍定稿与精修 | final 是可恢复的一遍定稿；同段更高有效 refined 成为当前正文。重复、乱序或迟到事件不得回滚正文或产生第二段。 | `final` 不表示永不变化；refined 不表示新增段落；不得原地抹除旧事实。 | revision 局部回归 + J1/J2/J5/J6。 | 核心链路、Gateway 恢复组合、默认产品 SQLite-only 旅程及历史复盘联合旅程均覆盖 final→refined 单投影；真实历史窗口交互与产品 smoke 仍待后续。 |
...
| **SEM-F11** | 带时间戳的字幕历史 | 字幕 MVP 必须自动保存已定稿正文；活动会话只在实时字幕窗显示，正常结束或被中断后才进入历史列表，并能按会话查看稳定时间线。历史与 txt/md/srt 读取同一当前正文投影；Agent 派生物存在时必须与原文分层展示。 | 能写 JSONL、能导出文件或只在当前字幕窗看到文字，都不等于历史查看已完成；“回放”只表示文本复盘，不表示音频播放。 | J1/J2；迁移后 J10；两小时 J8。 | 会话列表、终态过滤、带相对/墙钟时间的历史窗口及 SQLite 当前投影 txt/md/srt 导出已实现；确定性联合旅程已覆盖 mic→停止→复盘→导出→loopback XOR 新会话和稳定分页。真实 Electron 窗口交互与 J8 尚未验收，状态为“实现完成/尚未实机验收”。 |
| **SEM-F12** | 暂停、恢复与崩溃 | pause/resume 保留同一会话；迟到结果按 revision/watermark 处理。ASR 或 storage worker 重启后，已提交字幕事实可恢复；Agent 待办后续必须可幂等重放。运行 worker 故障后即使用户不点重试/停止，也必须主动释放采集资源。 | 不允许为了恢复接受跨会话事件；不允许在 error 状态继续占用 mic/loopback；Agent 恢复不得反向阻塞字幕恢复。 | J5/J6 + DB1 + I2 故障 smoke。 | realtime/Gateway 恢复与 Coordinator barrier/压力回归已通过；默认产品冷启动会原子地把遗留 active 会话收束为 interrupted，`before-quit` 在停采集、字幕/close ACK、worker shutdown 后才放行，并有有界强制终止失败路径。真实产品 Electron 退出 smoke、I3/历史与 Agent 仍待补。 |
...
| **SEM-F14** | 永不保存原始音频 | 原始 PCM、录音片段或可回放音频文件只允许在有界内存缓冲中服务实时 ASR/精修，使用后释放；产品、诊断和 smoke 现在及未来都不持久化现场采集音频。 | 不允许在 SQLite BLOB、应用数据目录、日志、导出、崩溃恢复文件、测试产物或 Agent 上下文中保存现场音频；不预留“以后打开录音”的隐藏路径。仓库可保留来源明确的静态合成测试语料。 | J12 + 存储/schema/诊断接口审查 + I4 打包版数据目录检查。 | diagnostic、schema/RPC、Gateway、默认产品生命周期和历史复盘/三格式导出联合旅程均验证无音频产物、字段或路径且无 JSONL 双写；I4 打包版应用目录仍待补，DB6 总门禁未完成。 |
...
| **SEM-T04** | 失败路径 | 每项用户能力至少覆盖一个关键失败或降级路径；核心能力必须 fail closed 或显式降级。 | 只覆盖 happy path 不得标记验收完成。 | 场景矩阵中的故障旅程或实机故障注入。 | SQLite 事实、Gateway 队列、DB2 第二文件中断、产品启动迁移失败清理和退出超时强制终止已有回归；历史层覆盖活动会话拒绝、越权载荷拒绝和取消导出不写文件。模型安装与 Agent 失败路径待实现，向量已后置。 |
