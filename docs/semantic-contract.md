...
| **SEM-F11** | 带时间戳的字幕历史 | 字幕 MVP 必须自动保存已定稿正文；活动会话只在实时字幕窗显示，正常结束或被中断后才进入历史列表，并能按会话查看稳定时间线。历史与 txt/md/srt 读取同一当前正文投影；Agent 派生物存在时必须与原文分层展示。 | 能写 JSONL、能导出文件或只在当前字幕窗看到文字，都不等于历史查看已完成；“回放”只表示文本复盘，不表示音频播放。 | J1/J2；迁移后 J10；两小时 J8。 | 会话列表、终态过滤、带相对/墙钟时间的历史窗口及 SQLite 当前投影 txt/md/srt 导出已实现；确定性联合旅程已覆盖 mic→停止→复盘→导出→loopback XOR 新会话和稳定分页，真实 Electron 壳已验证停止后打开终态历史。系统保存对话框/长列表人工交互、J8/I3 与 I4 仍待。 |
| **SEM-F12** | 暂停、恢复与崩溃 | pause/resume 保留同一会话；迟到结果按 revision/watermark 处理。ASR 或 storage worker 重启后，已提交字幕事实可恢复；Agent 待办后续必须可幂等重放。运行 worker 故障后即使用户不点重试/停止，也必须主动释放采集资源。 | 不允许为了恢复接受跨会话事件；不允许在 error 状态继续占用 mic/loopback；旧 native worker 未确认退出前不得启动 replacement；Agent 恢复不得反向阻塞字幕恢复。 | J5/J6 + DB1 + I2 故障 smoke。 | realtime/refine/storage UtilityProcess 已接 fatal `error`、角色标签和 exact-child exit 屏障；Coordinator 在旧世代 retirement 完成前禁止 replacement 启动。三轮真实批准模型并发载入/收束共 6 个 worker 均优雅 exit 0，补丁后三轮完整产品壳也为 0 crash；默认产品 SQLite 恢复/退出屏障和全量组合 CI 通过。用户截图的 `0x80000003` 仍未归因；硬崩溃重启、I3 与 Agent 仍待补。 |
