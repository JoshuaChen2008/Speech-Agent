...
| **SEM-F04** | 一遍定稿与精修 | final 是可恢复的一遍定稿；同段更高有效 refined 成为当前正文。重复、乱序或迟到事件不得回滚正文或产生第二段。 | `final` 不表示永不变化；refined 不表示新增段落；不得原地抹除旧事实。 | revision 局部回归 + J1/J2/J5/J6。 | 核心链路、Gateway 恢复组合和默认产品 SQLite-only 联合旅程已覆盖 final→refined 单投影；完整历史 UI 与实机产品 smoke 仍待后续。 |
...
| **SEM-F11** | 带时间戳的字幕历史 | 字幕 MVP 必须自动保存已定稿正文，并能按会话查看稳定时间线；历史与 txt/md/srt 读取同一当前正文投影。Agent 派生物存在时必须与原文分层展示。 | 能写 JSONL、能导出文件或只在当前字幕窗看到文字，都不等于历史查看已完成；“回放”只表示文本复盘，不表示音频播放。 | J1/J2；迁移后 J10；两小时 J8。 | 默认产品已自动把 final/refined 保存到 SQLite 当前投影，确定性重启查询已验证；会话列表、历史 UI、SQLite txt/md/srt 导出和 J8 未完成，因此字幕 MVP 尚未闭环。 |
...
| **SEM-F14** | 永不保存原始音频 | 原始 PCM、录音片段或可回放音频文件只允许在有界内存缓冲中服务实时 ASR/精修，使用后释放；产品、诊断和 smoke 现在及未来都不持久化现场采集音频。 | 不允许在 SQLite BLOB、应用数据目录、日志、导出、崩溃恢复文件、测试产物或 Agent 上下文中保存现场音频；不预留“以后打开录音”的隐藏路径。仓库可保留来源明确的静态合成测试语料。 | J12 + 存储/schema/诊断接口审查 + I4 打包版数据目录检查。 | diagnostic、schema/RPC、Gateway 与默认产品冷启动/迁移/XOR 会话/退出联合旅程均验证无音频产物、无音频字段或路径且无 JSONL 双写；历史导出与 I4 仍待补，DB6 总门禁未完成。 |
...
| **SEM-T04** | 失败路径 | 每项用户能力至少覆盖一个关键失败或降级路径；核心能力必须 fail closed 或显式降级。 | 只覆盖 happy path 不得标记验收完成。 | 场景矩阵中的故障旅程或实机故障注入。 | SQLite 事实、Gateway 队列、DB2 第二文件中断、产品启动迁移失败清理和退出超时强制终止已有回归；历史 UI、模型安装与 Agent 失败路径待实现，向量已后置。 |
