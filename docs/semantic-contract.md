...
| **SEM-F04** | 一遍定稿与精修 | final 是可恢复的一遍定稿；同段更高有效 refined 成为当前正文。重复、乱序或迟到事件不得回滚正文或产生第二段。 | `final` 不表示永不变化；refined 不表示新增段落；不得原地抹除旧事实。 | revision 局部回归 + J1/J2/J5/J6。 | 核心链路与 J1/J2 已覆盖；J5/J6 确定性联合旅程已覆盖暂停→refined→worker 故障→同会话恢复→文本存档，SQLite/storage worker 部分待 B3.3。 |
...
| **SEM-F07** | 权威持久化 | 字幕系统的目标唯一权威存储是 SQLite：不可变字幕事件是事实，segments 是可重建当前投影。B3.3 验收前，现有 JSONL 只是已实现过渡基线；迁移后只作导入导出/恢复。 | 不允许 SQLite/JSONL 长期双写；Agent 产物、向量、UI 状态或单个可变 transcript 行都不是字幕事实来源。 | DB0–DB2 + J1/J2 在 SQLite 后端重跑 + J10。 | DB0 开发态与 DB1 原子/幂等门禁已通过真实 Electron utility process；打包态 DB0、worker 自动恢复、DB2、产品接线与权威切换仍未完成，当前 J1/J2 继续验证 JSONL。 |
...
| **SEM-F12** | 暂停、恢复与崩溃 | pause/resume 保留同一会话；迟到结果按 revision/watermark 处理。ASR 或 storage worker 重启后，已提交字幕事实可恢复；Agent 待办后续必须可幂等重放。运行 worker 故障后即使用户不点重试/停止，也必须主动释放采集资源。 | 不允许为了恢复接受跨会话事件；不允许在 error 状态继续占用 mic/loopback；Agent 恢复不得反向阻塞字幕恢复。 | J5/J6 + DB1 + I2 故障 smoke。 | 真实结构 smoke 与确定性 J5/J6 已验证 realtime worker 故障后停止隐藏采集、重试恢复、暂停/精修和同一 JSONL 会话连续性；storage worker/SQLite 恢复待 B3.3，Agent 不属于本轮 MVP。 |
...
| **SEM-F14** | 永不保存原始音频 | 原始 PCM、录音片段或可回放音频文件只允许在有界内存缓冲中服务实时 ASR/精修，使用后释放；产品、诊断和 smoke 现在及未来都不持久化现场采集音频。 | 不允许在 SQLite BLOB、应用数据目录、日志、导出、崩溃恢复文件、测试产物或 Agent 上下文中保存现场音频；不预留“以后打开录音”的隐藏路径。仓库可保留来源明确的静态合成测试语料。 | J12 + 存储/schema/诊断接口审查 + I4 打包版数据目录检查。 | 产品诊断拒绝音频落盘；DB0/DB1 已检查 SQLite schema 与 storage RPC 无 BLOB/音频字段、报告不含正文/路径。产品切换后的应用目录、迁移/导出与 I4 打包版检查仍待补，因此 DB6 总门禁未完成。 |
