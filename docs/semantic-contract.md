...
| **SEM-F17** | 本地模型资源闭环 | 字幕模型只能由主进程依据内置、版本化且审计过的 manifest 下载：固定 HTTPS 来源、字节数、SHA-256、归档根、期望文件和上游出处；支持 `.part` 续传，经 staging 校验后同卷原子安装并写 ready marker。模型资源 UI 闭环必须从真实设置 renderer 的缺失状态点击下载，经受限 preload/IPC 进入主进程，并在完整字幕 bundle 就绪后让当前空闲应用无需手工改路径即可发布字幕 capability。 | renderer 不能提供 URL、哈希、模型/数据库路径、解压参数或任意文件写；直接调用 ModelManager、把资源页预置成 ready、“文件存在”“下载到 100%”或“开发仓库有模型”都不等于模型资源 UI 闭环；模型下载不是 Agent/翻译能力。 | J14 + I4；J14 的托管 CI 使用受控资源/ASR 外部边界，但必须保留真实 settings DOM、preload、IPC、ModelManager、热启用、SQLite 历史；真实批准模型调用证据沿用/扩展 I2。 | B4 UI 联合旅程已通过：隔离 userData 从 settings DOM 的 `missing` 状态点击下载，经历 preload/IPC、生产 ModelManager、Range、三资源 marker、空闲热启用、字幕、暂停/恢复、停止和 SQLite 历史；受控资源/fake ASR 边界另行标注。批准 270,938,600 字节 bundle 的真实安装及在线 ASR/离线精修/VAD 调用也有独立证据。干净机公网 I4 仍未验收。 |
