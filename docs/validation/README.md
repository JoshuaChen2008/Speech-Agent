...
| B5 打包态确定性资格 | 联合验收完成 | `npm run package:release` 与 B5 verifiers | 允许 Electron/NSIS 子进程的 Windows | 否 | 当前本地候选 SHA=`d862c5fc…0de10`、产品载荷 SHA=`a1f03ed6…9accc`；[`b5-packaging.md`](b5-packaging.md) 与七份 B5 JSON 已同轮绑定 | 当前 revision 推送后生成并下载复核新的远端 provenance；不得替代 I4 |
| J9-CI 远端资格 | 联合验收完成 | [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml) | GitHub Windows hosted runner | 否 | run `30766172580` 绑定 revision `f05f3f95…`，Electron `43.2.0` 前置、全部 workflow steps 与 605 项回归成功；artifact `qualification-evidence-f05f3f95…-30766172580-1` 下载后通过五个 strict readers、报告/lockfile/workflow SHA、跨报告绑定及 41 文件/29 JSON 隐私负扫描。下载包不含 installer 字节，同轮 workflow 已在索引生成前复核其精确 installer；该证据不绑定当前本地候选 | 当前候选推送后生成并下载复核新的远端 provenance；不得替代 J15a/I2/I3/I4 实机门禁 |
