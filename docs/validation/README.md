...
> 2026-08-07 复核基线为 revision `a91a200cc2d3df5f73dcb7a0894a9758e532106a`：正常 Windows 子进程权限下 `npm test` 为 core 414 tests=407 pass+7 expected skips、integration 27/27、evidence 200/200；总计 641 tests=634 pass+7 expected skips+0 fail。最近一次可下载并经下载侧复核的 J9-CI 联合验收完成基线为 run `30792514100` / revision `2890dd9a5a224bca75cff2257cfd14013d74584d`；artifact ID `8847650958`、GitHub ZIP digest `9e4b2d7b9975afc288c5a14728e0a8812ec00eea1f6a25f46080063c8beccafd`、索引绑定 installer SHA `25ff62a88efb8e8d021887df3a2685c72515baadca23f4810902f13d2ae19b76`。当前 revision 的 run `30801570384` 在 evidence lane 得到 197 pass+3 fail，最终 provenance 索引按设计跳过；`scripts/gate-0b/corpus.json` 缺少 LF checkout 规则，hosted CRLF SHA `fdf4420a243cc6e0efe0074dbf5c60c97efc1d1284448b7bfce9c3d658e1fce9` 与登记的 LF SHA `7edd6dff286b84619a3b68f385ba04622103ffe17cc57dbe5e1f16521deb156d` 不同，因此当前 revision 的 J9-CI 为实现完成·尚未验收。产品载荷输入自 `2d3d6bdf5d745c15c239ef7503a0dfac211409a8` 起未变化，本机 B5 候选仍绑定当前产品载荷。
...
| B5 打包态确定性资格 | 联合验收完成 | `npm run package:release` 与 B5 verifiers | 允许 Electron/NSIS 子进程的 Windows | 否 | 当前本地候选 SHA=`d862c5fc…0de10`、产品载荷 SHA=`a1f03ed6…9accc`；[`b5-packaging.md`](b5-packaging.md) 与七份 B5 JSON 已同轮绑定 | 用该精确本机安装器进入 I4；不得用远端索引中的另一安装器摘要替换 |
...
| J9-CI 当前 revision 复核 | 实现完成·尚未验收 | run `30801570384` / revision `a91a200cc2d3df5f73dcb7a0894a9758e532106a` | GitHub Windows hosted runner | 否 | 产品与打包前置成立；evidence 200 tests=197 pass+3 fail，最终 provenance 索引跳过；diagnostic artifact ID `8851119368`、GitHub ZIP digest `8a2b7290f17a5337e69c43d2f0d2367d79aef1a3b2a5cf40df4e3076ed30c56c` | 为 Gate 0B corpus 与其严格复算输入补齐 LF checkout 合同，重跑完整 J9-CI；不得沿用本机 642 tests=635 pass+7 expected skips+0 fail 冒充远端资格 |
...
| Gate 0B 模型判定 | 已决定 | Gate 0B 脚本与指标回归 | 已记录机器基线 | 仅受控语料 | [`gate-0b.md`](gate-0b.md)、[`gate-0b-realtime-candidate-summary.json`](gate-0b-realtime-candidate-summary.json)；严格汇总绑定 registry `d202c018…bdcac`，三个新候选均因内容质量失败保持 `evaluation-only`，0 个 eligible ID，尚未选定替代模型，生产 manifest 未变 | 另行登记新的模型资产或识别架构；保持全部冻结门槛后复测，不得用 Gate 0B 替代两来源 I2 |
