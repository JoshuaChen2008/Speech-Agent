...
| J9-CI 远端资格 | 实现完成·尚未验收 | [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml) | GitHub Windows hosted runner | 否 | run `30764235663` 绑定 revision `92a968a0…`，已通过 Electron `43.2.0`、存储、四窗口产品壳、packaged 双启动、release NSIS 与隔离安装卸载；完整回归仅 I3 非音频 Markdown 标题因外部时区不同而有 1 项实际字节 SHA 漂移，provenance 索引按设计跳过。I3 fixture child UTC 前置为实现完成·尚未验收，产品导出仍跟随 Windows 系统时区 | 推送 I3 fixture child UTC 前置，取得成功 workflow 与 provenance artifact |
