...
| J9-CI 远端资格 | 实现完成·尚未验收 | [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml) | GitHub Windows hosted runner | 否 | run `30765231206` 绑定 revision `3b4b0db2…`，Electron `43.2.0` 前置、完整 workflow、provenance 绑定与 artifact 上传成功；下载后的 strict reader 因 caption layout runner/verifier 的 hosted CRLF 与本机 LF 不同而拒绝复算。两份脚本及索引复算的 lockfile/workflow LF checkout 修复为实现完成·尚未验收，产品导出仍跟随 Windows 系统时区 | 推送全部四个 provenance 输入的 LF checkout 修复，取得新成功 workflow，并对下载 artifact 执行 strict reader 与隐私复核 |
