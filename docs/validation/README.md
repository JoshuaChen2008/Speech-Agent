...
| I2 真实来源与交互恢复 | 实现完成·尚未验收 | `run-i2-live-series.ps1`、`run-i2-interaction.ps1` | 具备真实 `loopback`/`mic`、设备控制权与睡眠权限的 Win11 | 是 | [`i2-real-source-series.md`](i2-real-source-series.md)、[`i2-live-b96b8fe-loopback/`](i2-live-b96b8fe-loopback/)、[`i2-interaction-recovery.md`](i2-interaction-recovery.md)；最近 `loopback` 五轮冻结 P95=1242ms，逐轮 trace 已决定重新开启 Gate 0B 实时模型替换评估，替代模型尚未选定 | 保持 `<1000ms`、`source t0 + 140ms`、Silero 与 4/12 边界；先选定并登记替代模型，再重跑两来源各五轮，并完成原生拖动、设备移除/Retry、睡眠/唤醒/Retry 与物理麦克风证据 |
