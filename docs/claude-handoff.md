...
> B5 已从当前源码重建、重取证并冻结未签名安装器 SHA `4abc23bc…b31de`；package layout、native/SQLite utility、packaged 首启/同 `userData` 离线复启、核心/精修分层、下载取消/复启继续、J15b/J15c 产品旅程、两次 clean exit、旧档迁移/三格式导出和隔离 NSIS 机械探针均有证据，同一 run ID、四份报告 SHA 与 114 文件产品载荷 SHA `b6503ca2…a0bbd` 已闭合绑定到 release layout。机械探针只证明无关 APPDATA 哨兵不变，不证明正式应用 `userData`。权威 I2 10 轮均有外部退出 sidecar；另一次未纳入 bundle 的运行
...
| B5 分发 | 当前源码确定性联合验收完成 / I4 待验 | SHA `4abc23bc…b31de` 的 ASAR/NSIS/native/SQLite、packaged 双启动、核心/精修分层、下载取消/复启继续、J15b/J15c 产品旅程、旧档迁移/导出、两次 exact-child exit、同轮/114 文件载荷 binding 与隔离安装卸载/无关 APPDATA 哨兵已取证；I4 非音频 runner/verifier 已备但没有干净机报告，正式应用 `userData` 仍未验，且候选未签名、真实权限/物理来源未验 |
...
### 12.5 A4 layout contract（MVP 后交互质量项）

`caption.css` 的 `.tb-hole` 固定约 `584 × 64px`；主进程工具条窗口和停靠尺寸由 `src/main.js` 决定。最坏情况洞已能保证工具条不被字幕窗吞掉点击，只是常态会多释放一小块透明穿透区，因此不阻断当前 MVP。后续精度优化可增加主进程只读 `onOverlap(cb)`，下发字幕窗 CSS px 的实际 overlap rect。
