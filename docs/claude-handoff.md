...
> B5 已从当前源码重建、重取证并冻结安装器 SHA `4a1deb35…2449dd`；package layout、native/SQLite utility、packaged 首启/同 userData 离线复启、两次 clean exit、旧档迁移/三格式导出和隔离 NSIS 机械探针均已通过，同一 run ID、四份报告 SHA 与 112 文件产品载荷 SHA `503a40df…b93d` 已闭合绑定到 release layout。机械探针只证明无关 APPDATA 哨兵不变，不证明正式应用 userData。权威 I2 10 轮均有外部退出 sidecar；另一次未纳入 bundle 的运行
...
| B5 分发 | 当前源码确定性资格通过 / I4 待验 | SHA `4a1deb35…2449dd` 的 ASAR/NSIS/native/SQLite、packaged 双启动、旧档迁移/导出、两次 exact-child exit、同轮/载荷 binding 与隔离安装卸载/无关 APPDATA 哨兵已取证；I4 非音频 runner/verifier 已备但没有干净机报告，正式应用 userData 仍未验，且候选未签名、真实权限/物理来源未验 |
