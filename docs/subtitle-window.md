...
工具条宽度的由来（V2）：窗口按**最坏情况**固定，条自适应，窗内**右对齐**。

- 最坏是 `error` 态（状态副行 + 停止 + 重试 + 下一步），Electron @125% DPI 实测自然宽 459；476 = 459 + 17 余量，留给不同 DPI 的字体度量差异和比样例更长的音频源标签。
- 常见态只有 359–391，多出来的透明区域逐像素穿透，看不见。
- 右对齐是关键：`dock()` 按 `窗口右沿 − TB_MARGIN` 反推位置，右对齐后这个假设与实际渲染恒等，**条多宽都不影响停靠精度**，公式一行没改。
- 超出余量时状态副行打省略号；`.bar` 是 `flex-wrap: nowrap`，`.bar-group` 是 `flex: none`，保证挤压只落在状态文字上，不会把「重试 / 下一步」这些出口压没，也不会换行成两层条。
