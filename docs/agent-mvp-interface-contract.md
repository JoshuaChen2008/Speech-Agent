...
D12 增加 UI-free 的 main-owned `FormalAgentRuntime` 与 `MeetingStoppedPersistenceSink`。runtime 只读取真实 `ConfigStore.get()`、`AgentProviderBootstrap.getEligibilityProviderFacts()` 和受信任 `localModelReady`，选择并冻结 exact 十字段 `AgentEligibilityContext`；`updateAgentSettings` 串行执行 ConfigStore revision 更新与 `agent.applyTaskPolicy`，每次尝试都先清除旧 ready/revision，只有当前 storage worker 代次确认策略后才重新标为可领取。非法资格事实或策略写入失败必须 fail closed；后续 `recoverTerminalSessions({ sessionIds })` 必须先重放当前策略再逐会话对账。恢复输入只接受 0–256 个不重复非空 `sessionId`，不接受正文、来源、时间、provider、URL、模型或凭据。
...
D15 冻结 main-owned `FormalAgentJobScheduler` 与字幕会话资源桥接的窄职责。调度器只驱动真实 `AgentJobRunner`，同一时刻至多执行一个由 runner 建立并拥有的 logical claim attempt。attempt 冻结 claim idempotency key、owner、请求 `leaseMs`、`availableTaskKinds`、`localWorkAllowed` 与请求摘要；receipt 返回的任务 lease 不属于调度器请求身份，仍由 runner/storage 权威管理。`runNext` 未确定收束时调度器必须保留并在下一次显式唤醒复用同一 attempt，确定返回 receipt/空结果后才清除并建立下一 attempt。策略应用、终态会话对账、可重试到期、utility/storage replacement 与资源重新允许都只递增 `wakeEpoch`；scheduler 已 idle 时 `wake()` 必须排队一个且仅一个 drain，drain 捕获 epoch，空扫描进入 idle 前由同一 owner 临界点再次比较，变化时继续扫描。调度器为不可重启的 one-shot lifecycle：`start` 只能成功一次，`stop` 为终态，清除 pending wake/重试 timer 并推进 generation，任何旧 timer 或微任务都不得再次领取；运行中的调用由外层应用按既有 Agent utility 退出语义收束。runner 或 transport 异常只发布 exact observer payload `{ code: 'AGENT_SCHEDULER_FAILED' }`，不写 `agent_jobs.error_code`，pending attempt 与任务 lease 恢复继续由 runner/storage 的既有闭集语义负责。

字幕会话资源桥接必须先调用 `SessionCoordinator.onSnapshot` 建立订阅，再读取一次 `getSnapshot()`，并只应用 revision 单调不减的 exact `{ revision, sessionId }` 投影，避免订阅与初始读取之间丢状态。`sessionId !== null` 覆盖开始、监听、暂停、停止、故障和恢复中的活动会话；它只让下一 logical claim attempt 冻结 `localWorkAllowed=false`，`providerKind='cloud'` 的任务仍可执行。bridge `stop` 必须取消订阅并以 generation guard 忽略已经排队的旧回调。D15 不声称 snapshot 与跨进程 claim 原子化，也不改变 storage worker 的 FIFO、策略或租约权威；本地 Agent 模型 provider、运行中本地任务抢占、正式 main/preload/renderer 与真实 DeepSeek HTTP 均后置。决策见 ADR 0012。

当前 D15 后台 Agent 任务调度与会话资源桥接为已决定。
