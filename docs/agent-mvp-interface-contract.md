...
D12 增加 UI-free 的 main-owned `FormalAgentRuntime` 与 `MeetingStoppedPersistenceSink`。runtime 只读取真实 `ConfigStore.get()`、`AgentProviderBootstrap.getEligibilityProviderFacts()` 和受信任 `localModelReady`，选择并冻结 exact 十字段 `AgentEligibilityContext`；`updateAgentSettings` 串行执行 ConfigStore revision 更新与 `agent.applyTaskPolicy`，策略写入失败时不得把 runtime 标为可领取，后续 `recoverTerminalSessions({ sessionIds })` 必须先重放当前策略再逐会话对账。恢复输入只接受 0–256 个不重复非空 `sessionId`，不接受正文、来源、时间、provider、URL、模型或凭据。

`MeetingStoppedPersistenceSink` 完整代理现有字幕 persistence sink，且保持其 open/append/close/retry/flush 的返回与错误语义。只有底层 `closeSession` 或携带已冻结终态 close 的 `retry` 成功后，才以 exact `{ sessionId }` 调用 `FormalAgentRuntime.notifyMeetingStopped`；该调用只启动 detached reconciliation，不能被 `SessionCoordinator.stop()` 等待。runtime 对同一会话的 in-flight 通知只执行一次，完成或失败后释放内存身份；失败只向 main observer 投影 stable `AGENT_RECONCILE_FAILED`，不保留原始 Error。应用退出不等待该尽力通知；下次启动或 storage worker replacement 用同一恢复入口补建，SQLite dedupe 仍是最终权威。D12 不把任务执行、Agent 模型 provider 调用或 Agent utility 接入字幕停止路径。
