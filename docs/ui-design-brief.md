...
**稳定悬浮表面**：工具条在未锁定、悬停、聚焦、拖动、锁定与 `attention` 状态下都保持同一套深色半透明表面和浅色普通控件；深色、浅色与自动主题不得翻转它的底色、按钮、两列三行六点握把或 phase 色调。`toolbarOpacity` 只调整表面透明度；即使用户明确调到全透明，浅色图标仍以固定深色光晕维持跨背景可见性。Windows 系统高对比仍可接管颜色。
...
- 整个工具条真实外接轮廓（含状态文字和控件间隙）都从字幕抓取面排除；工具条自身只允许从明确可见的握把开始拖动，按钮、状态文字和空隙不承担隐藏拖动语义。
- 握把使用 Fluent System Icons 的两列三行六点图形，保持既有 `24 × 30 DIP` 命中盒；图形替换不得改变拖动时机、窗口 bounds 或外接轮廓。
...
- 字幕命中必须使用工具条当前真实外接矩形；`584 × 64` 只允许在首帧、renderer reload、布局未就绪、非法或陈旧矩形时临时 fail-safe，收到同代有效矩形后立即收缩。
