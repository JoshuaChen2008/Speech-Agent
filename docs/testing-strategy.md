...
| J9-CI | 打包态确定性资格：显式供给并前置校验锁定 Electron runtime；正式 ASAR/NSIS 内容，native utility 实际加载，受控首启/复启，exact-child 正常退出，同轮/载荷证据绑定，精确候选隔离安装/卸载；最终 provenance 索引再绑定 exact checkout revision 与 GitHub run；全程不需要 Agent | B5 每次 PR 的 Windows package lane | 当前本机候选已达到确定性联合验收完成：168 个 ASAR 条目、114 文件产品载荷 SHA=`b6503ca2…a0bbd`、29 个入口、5 个 native 与负扫描；packaged 双启动、七份报告同轮哈希链、SHA=`4abc23bc…b31de` 的未签名 NSIS 安装/卸载及无关 APPDATA 哨兵均有本地证据。远端 run `30750568366` 已绑定 revision `2242103eb917f2afbfe81c7c8df788852bb36ebc`，但在 `npm ci` 后因 `electron.exe` 缺失而于首个字幕布局步骤失败，后续组合门禁与最终 provenance 索引未执行。显式 runtime 供给和前置版本校验已实现，并由本地真实 `43.2.0` 版本探针及 workflow 顺序契约覆盖；该修复与远端 CI 闭环仍为实现完成·尚未验收。正式应用 `userData` 和干净机 I4 尚未达到实机验收完成。 |
...
2026-08-03 当前工作树为 core 410/410、integration 27/27、evidence 163/163，共 600/600；另有当前源码对应的 `caption-layout-report@v2` 真实 Chromium 布局资格 15/15 场景、16/16 不变量，包含 identity-only 视口淘汰回报、容量保险墓碑及迟到高版本 `final` 不复活，报告边界仍明确保持 `humanVisualReview=false`、`dpiMatrix=false`、`audioCapture=false`。可见 DWM 资格基础设施的 9 项契约测试已计入 evidence；由于当前尚无 36 组合人工观察矩阵，它仍为实现完成·尚未验收。远端 run `30750568366` 绑定 revision `2242103…`，但在 `npm ci` 后因 Electron runtime 缺失而于首个布局步骤失败，后续组合门禁没有执行。Windows 系统 `tar.exe` 与 Electron child 在受限沙箱内可能被 `EPERM` 拒绝，相关 lane 必须在允许启动这些精确子进程的 Windows runner/本机运行；这属于执行环境前提，不能把沙箱失败计作产品断言通过或失败。
