...
| J2 | 听写模式：点击运行 → 麦克风字幕 → partial/final/refined → 自动持久化 → 停止/重启 → 按时间戳查看历史 → 导出 | 字幕 MVP / 发布阻断；真实麦克风另走 I2 smoke | 历史复盘联合旅程已覆盖 mic 单路 start、partial 排除、final→refined 当前投影、正常停止、带双时间戳详情和三格式导出，随后切换 loopback 仍保持 XOR/会话隔离。真实 Electron 产品壳用显式 fake-ASR 覆盖 UI/SQLite；`physical-preferred-label-heuristic` 声学 fixture 另以同轮 Gate 0C 精确报告与匿名输入/输出标签绑定后完成 5/5 轮 ASR/refine，最大 final/refined CER=0.035714/0、12 项丢失峰值全为 0，冻结字幕可见延迟 P50/P95/min/max=875/1005/822/1005ms，P95 超线 5ms。该标签启发式不是硬件证明；两来源性能与完整产品重启/历史链路仍未通过，因此 I2/I4 都未关闭。 |
...
| J6 | realtime/refine/storage worker 崩溃后恢复；已定稿内容仍可显示、落盘、历史可见；Agent 可继续追赶 | CI 故障注入 + I2/I3 实机 smoke | realtime/Gateway 恢复与字幕/close ACK 压力旅程已通过；UtilityProcess fatal error 由固定角色诊断消费，worker 按 30 秒 graceful + 5 秒 exact-child reap 收束，字幕运行时 45 秒触发升级且 ModelManager 5 秒并行；升级后仍等待 exact child，无法收殓旧代时 Coordinator 禁止启动 replacement generation。批准模型活跃诊断三轮的 6 个 worker 全部优雅 exit 0、fatal 0；当前 exit-bound 权威包的两来源 10 轮均有 report 绑定的外部 exact-child exit-0 sidecar，且 12 项丢失峰值全为 0。该 sidecar 防止内部先写 `pass`、随后悬挂/超时被误判绿色，但不证明硬崩溃恢复或崩溃根因。另一次非权威 run 在 pass 后悬挂并留下 `PostQueuedCompletionStatus(6)`；只已证明 `PostQueuedCompletionStatus(6) → uv_fatal_error → DebugBreak` 的 `0x80000003` 即时机制，因无 native stack，具体竞态、发送者和进程角色均未证。120 秒 exact-process timeout 只防无限等待并判失败。真实硬崩溃→重启恢复、I3 与 Agent 仍待补，完整 J6 未通过。 |
...
与该冻结输入诊断分开，当前 I2 exit-bound bundle 已让 loopback/mic 各跑 5 轮 Electron audio-host→online ASR→offline refine→外部 exact-child 退出观察。10/10 轮都有 final/refined，且各自 report 均绑定 `exited-zero-without-runner-termination` sidecar；loopback 最大 final/refined CER=0/0、mic=0.035714/0，所有 captured/sent/ingested 帧一致，12 项丢失峰值全为 0。冻结字幕可见延迟 P50/P95/min/max：loopback=1133/1158/1092/1158ms，mic fixture=875/1005/822/1005ms；final-after-end P95 分别 710/792ms。六段诊断 P95 依次为 loopback=729/405/1/33/1/1ms、mic=557/500/1/28/1/1ms，说明主要预算位于采集/VAD 与触发所需音频，而非 IPC。两来源分别超冻结 `<1000ms` 线 158ms/5ms；再加上交互与恢复场景未齐，I2 整体门禁仍未关闭。5 轮 happy-path 也不替代真实 pause/refine、拖动、设备变化、睡眠/唤醒、硬崩溃、I3 或 I4。
