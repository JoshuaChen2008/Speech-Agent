...
2026-08-03 当前工作树为 core 410/410、integration 27/27、evidence 168/168，共 605/605；另有当前源码对应的 `caption-layout-report@v2` 真实 Chromium 布局资格 15/15 场景、16/16 不变量，包含 identity-only 视口淘汰回报、容量保险墓碑及迟到高版本 `final` 不复活，报告边界仍明确保持 `humanVisualReview=false`、`dpiMatrix=false`、`audioCapture=false`。可见 DWM 资格基础设施的 9 项契约测试已计入 evidence；由于当前尚无 36 组合人工观察矩阵，它仍为实现完成·尚未验收。run `30765231206` 在 revision `3b4b0db2f56f57e3d0e7500c77593b406a5dd857` 上完成全部 603 项回归、provenance 绑定与 artifact 上传；下载后的 strict reader 随后因 caption layout runner/verifier 的 hosted CRLF 与本机 LF 不同而拒绝当前工作树复算。两份脚本及索引复算的 lockfile/workflow 已由 `.gitattributes` 固定 LF，并有回归锁定全部 strict reader 文本 provenance 输入；该修复为实现完成·尚未验收，仍待新 revision 的 workflow 与下载 artifact strict reader 复验。Windows 系统 `tar.exe` 与 Electron child 在受限沙箱内可能被 `EPERM` 拒绝，相关 lane 必须在允许启动这些精确子进程的 Windows runner/本机运行；这属于执行环境前提，不能把沙箱失败计作产品断言通过或失败。
