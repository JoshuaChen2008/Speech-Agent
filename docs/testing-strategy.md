...
| 模型/音频实机 smoke | `scripts/model-install-live-smoke.js`、`scripts/i2-live-caption-smoke.js`、`scripts/run-i2-live-series.ps1` + 两个 I2 verifier | 有批准模型或可播放/采集音频的 Windows 机器 | 真实模型安装/调用；loopback/mic 分路 ASR、VAD、refine、匿名标签哈希、资源/传输指标和每来源固定 5 轮 P50/P95/min/max；自动 mic fixture 先由 memory-only Gate 0C 预检并标记为 `physical-preferred-label-heuristic`，不作硬件证明 |
...
| J2 | 听写模式：点击运行 → 麦克风字幕 → partial/final/refined → 自动持久化 → 停止/重启 → 按时间戳查看历史 → 导出 | 字幕 MVP / 发布阻断；真实麦克风另走 I2 smoke | 历史复盘联合旅程已覆盖 mic 单路 start、partial 排除、final→refined 当前投影、正常停止、带双时间戳详情和三格式导出，随后切换 loopback 仍保持 XOR/会话隔离。真实 Electron 产品壳用显式 fake-ASR 覆盖 UI/SQLite；`physical-preferred-label-heuristic` 声学 fixture 另以同轮 Gate 0C 精确报告与匿名输入/输出标签绑定后完成 5/5 轮 ASR/refine，最大双 CER 0、12 项丢失峰值全为 0，P50/P95/min/max=843/1024/819/1024ms。该标签启发式不是硬件证明；完整产品重启/历史链路仍归 I4。 |
...
| J12 | 隐私负证据：正常停止、崩溃恢复、诊断 smoke、迁移、模型安装和导出后，SQLite、应用数据目录、日志、测试产物与 Agent 上下文均不存在现场采集 PCM/WAV、录音片段或音频路径 | 字幕 MVP PR schema/文件检查 + J14 安装目录检查 + I2 diagnostic + I4 打包版数据目录检查；测试语料只跟踪 generator/reference，生成 WAV 被忽略 | diagnostic、schema/RPC、Gateway、默认产品两次冷启动/迁移/XOR 会话/退出、历史 txt/md/srt 导出、模型联合旅程及批准大模型真实安装均无现场音频产物、字段或路径且不创建新 JSONL；schema v4 I2 报告绑定生成 WAV/reference digest 但不含音频或路径。真实大归档验收发现并修复“整包解出上游示例 WAV”，现只提取 requiredFiles。I4 打包版仍待补 |
...
I2 实机入口 `scripts/i2-live-caption-smoke.js` 必须显式传入且只接受一个 `--source loopback` 或 `--source mic`，两次运行不得并发。schema v4 child 包含实际播放起止、冻结语料估算语音起点、匿名输入/输出标签绑定、字幕到达时序、Electron CPU/工作集、audio-host 队列/丢帧、worker 缺口与 CaptionEvent 边界丢弃计数，且不包含字幕正文、PCM、现场音频文件或音频路径。operator 朗读仍可用；自动 mic 模式读取同轮 Gate 0C memory-only 报告并绑定其精确 SHA，以唯一 label SHA 匹配输入并绑定输出标签。该 `physical-preferred-label-heuristic` fixture 只能防预检后静默换标签，不是硬件证明，也不能排除未知或伪造标签的虚拟设备。语料 WAV 由受跟踪的 generator/reference 本地生成且被忽略；child 同时绑定 WAV 与 reference digest。

`scripts/run-i2-live-series.ps1` 对每来源固定跑 5 轮，每轮先经严格递归 `verify-i2-live-report.js`，再由 `summarize-i2-live-series.js` 生成并自校验确定性 summary。原始 UTF-8 JSON 在对象校验前即拒绝 BOM、非法编码、重复键（包括转义后等价键）、非有限数值和尾随输入；Gate、child 与 summary 随后都走闭合字段验证。权威 bundle 是 [`validation/i2-live-v4/`](validation/i2-live-v4/) 中 SHA-256 为 `43e97770e3508c88ff5843df2c897825f7e8b717bc1010fccb750c5beb2d1f0b` 的精确 Gate 0C preflight、loopback/mic 各 5 个 child 及两份内嵌 summary；CI 从这 11 个精确输入重建 summary，并要求与 tracked summary byte-for-byte 相同。托管 CI 只证明证据完整性，不重放或证明硬件。
...
与该冻结输入诊断分开，当前 I2 schema v4 bundle 已让 loopback/mic 各跑 5 轮 Electron audio-host→online ASR→offline refine→退出。10/10 轮都有 final/refined，两来源最大 final/refined CER 均为 0；所有 captured/sent/ingested 帧一致，12 项丢失峰值全为 0。估算语音起点首 partial 的 P50/P95/min/max：loopback=1112/1126/1042/1126ms，mic fixture=843/1024/819/1024ms；final-after-end P95 分别 722/772ms。两路产品路径 P95 都高于冻结的裸模型 `<1000ms` 线（mic 超 24ms），故 I2 性能债和整体门禁仍未关闭。由于截图没有可核验的发生时间或角色证据，不能把这些运行与历史 `0x80000003` 建立修复前后关系；5 轮 happy-path 也不替代真实 pause/refine、拖动、设备变化、睡眠/唤醒、硬崩溃、I3 或 I4。

普通 `npm start` 和产品壳旅程由 `scripts/run-supervised-electron.js` 监督唯一 exact child；main 只上报固定枚举的生命周期与角色级事件，报告不保存 PID、命令行、正文、音频、路径、stack 或 dump。受监督多窗口产品壳已得到 clean exit、0 incident、未观察到 breakpoint，同时产品旅程报告仍是 `partial`：它使用 fake ASR、受控模型 fixture（无真实张量）且没有打开物理音频或访问真实公网。现有 `0x80000003` 截图没有可核验的发生时间或 native stack，当前通过结果不能升级为根因证明或永久修复声明。完整边界见 [Electron breakpoint 调查记录](validation/electron-breakpoint-investigation.md)。
