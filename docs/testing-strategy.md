...
| J9-CI | 打包态确定性资格：显式供给并前置校验锁定 Electron runtime；正式 ASAR/NSIS 内容，native utility 实际加载，受控首启/复启，exact-child 正常退出，同轮/载荷证据绑定，精确候选隔离安装/卸载；确定性模型解析测试使用工作区内受控模型就绪证明 fixture，I3 文本源码 provenance 固定并规范为 LF；最终 provenance 索引再绑定 exact checkout revision 与 GitHub run；全程不需要 Agent | B5 每次 PR 的 Windows package lane | 当前本机候选已达到确定性联合验收完成：168 个 ASAR 条目、114 文件产品载荷 SHA=`704a3c6f…d7520`、29 个入口、5 个 native 与负扫描；packaged 双启动、七份报告同轮哈希链、SHA=`1d51dbdb…b9e6d` 的未签名 NSIS 安装/卸载及无关 APPDATA 哨兵均有本地证据。run `30761472817` 绑定 revision `811fc6d14e927305588cf3e1603a9d09f7186e18`，已通过 Electron `43.2.0`、字幕布局、DB0/DB1/Gateway、四窗口产品壳、packaged 双启动、release NSIS 与隔离安装卸载，hidden artifact 已上传；末端 evidence 回归因 I3 测试依赖开发机模型目录及文本源码行尾差异失败，provenance 索引正确跳过。工作区内受控模型就绪证明 fixture 与 LF provenance 修复现为实现完成·尚未验收；远端 CI 闭环仍为实现完成·尚未验收。正式应用 `userData` 和干净机 I4 尚未达到实机验收完成。 |
