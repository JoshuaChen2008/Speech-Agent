...
| J9-CI | 打包态确定性资格：显式供给并前置校验锁定 Electron runtime；正式 ASAR/NSIS 内容，native utility 实际加载，受控首启/复启，exact-child 正常退出，同轮/载荷证据绑定，精确候选隔离安装/卸载；最终 provenance 索引再绑定 exact checkout revision 与 GitHub run；全程不需要 Agent | B5 每次 PR 的 Windows package lane | 当前本机候选已达到确定性联合验收完成：168 个 ASAR 条目、114 文件产品载荷 SHA=`b6503ca2…a0bbd`、29 个入口、5 个 native 与负扫描；packaged 双启动、七份报告同轮哈希链、SHA=`4abc23bc…b31de` 的未签名 NSIS 安装/卸载及无关 APPDATA 哨兵均有本地证据。远端 run `30750568366` 已绑定 revision `2242103eb917f2afbfe81c7c8df788852bb36ebc`，但在 `npm ci` 后因 `electron.exe` 缺失而于首个字幕布局步骤失败，后续组合门禁与最终 provenance 索引未执行。显式 runtime 供给和前置版本校验为已决定；远端 CI 闭环仍为实现完成·尚未验收。正式应用 `userData` 和干净机 I4 尚未达到实机验收完成。 |
