...
| J9-CI | 打包态确定性资格：显式供给并前置校验锁定 Electron runtime；正式 ASAR/NSIS 内容，native utility 实际加载，受控首启/复启，exact-child 正常退出，同轮/载荷证据绑定，精确候选隔离安装/卸载；确定性模型解析测试使用工作区内受控模型就绪证明 fixture，I3 文本源码 provenance 与全部产品文本 checkout 固定 LF，但产品载荷、ASAR、installer 与报告仍按精确字节计算；最终 provenance 索引再绑定 exact checkout revision 与 GitHub run；全程不需要 Agent | B5 每次 PR 的 Windows package lane | 当前本机候选已达到确定性联合验收完成：168 个 ASAR 条目、114 文件产品载荷 SHA=`2045fb2c…5d459`、29 个入口、5 个 native 与负扫描；packaged 双启动、七份报告同轮哈希链、SHA=`5681f017…58cc3` 的未签名 NSIS 安装/卸载及无关 APPDATA 哨兵均有本地证据。run `30763123116` 绑定 revision `2efb8969ec0608dd44695586ef70255f01c79fa7`，已通过 Electron `43.2.0`、字幕布局、DB0/DB1/Gateway、四窗口产品壳、packaged 双启动、release NSIS 与隔离安装卸载，且模型 fixture 与十个 I3 文本 provenance 输入均通过；末端仅因完整产品载荷中其余文本 checkout 行尾未固定而有 2 项 I3 证据失败，provenance 索引正确跳过。完整产品文本 LF checkout 修复与新候选 B5 重建为实现完成·尚未验收；远端 CI 闭环仍待新 revision 验收。正式应用 `userData` 和干净机 I4 尚未达到实机验收完成。 |
