...
2026-08-03 当前工作树为 core 414/414、integration 27/27、evidence 168/168，共 609/609；另有当前源码对应的 `caption-layout-report@v2` 真实 Chromium 布局资格 15/15 场景、16/16 不变量，包含 identity-only 视口淘汰回报、容量保险墓碑及迟到高版本 `final` 不复活，报告边界仍明确保持 `humanVisualReview=false`、`dpiMatrix=false`、`audioCapture=false`。可见 DWM 资格基础设施的 9 项契约测试已计入 evidence；由于当前尚无 36 组合人工观察矩阵，它仍为实现完成·尚未验收。run `30768637180` 在 revision `2d3d6bdf5d745c15c239ef7503a0dfac211409a8` 上完成全部 609 项回归、provenance 绑定与 artifact 上传；下载 artifact 随后通过五个 strict readers、报告/lockfile/workflow SHA、跨报告绑定及 41 文件/29 JSON 隐私负扫描，当前源码 J9-CI 已达到确定性联合验收完成。下载包不含 installer 字节，同轮 workflow 已在索引生成前复核远端 installer SHA `91297945…19292`。Windows 系统 `tar.exe` 与 Electron child 在受限沙箱内可能被 `EPERM` 拒绝，相关 lane 必须在允许启动这些精确子进程的 Windows runner/本机运行；这属于执行环境前提，不能把沙箱失败计作产品断言通过或失败。
