...
| J9-CI | 打包态确定性资格：显式供给并前置校验锁定 Electron runtime；正式 ASAR/NSIS 内容，native utility 实际加载，受控首启/复启，exact-child 正常退出，同轮/载荷证据绑定，精确候选隔离安装/卸载；最终 provenance 索引再绑定 exact checkout revision 与 GitHub run；全程不需要 Agent | B5 每次 PR 的 Windows package lane | 当前本机候选已达到确定性联合验收完成：168 个 ASAR 条目、114 文件产品载荷 SHA=`b6503ca2…a0bbd`、29 个入口、5 个 native 与负扫描；packaged 双启动、七份报告同轮哈希链、SHA=`4abc23bc…b31de` 的未签名 NSIS 安装/卸载及无关 APPDATA 哨兵均有本地证据。run `30760407160` 绑定 revision `0d0cd9f91cfd5136bbd5d3fec44e636da04b4e21`，显式 Electron `43.2.0` 供给/版本校验、字幕布局与 DB0 均已执行，随后因 DB1 旧 fixture 未冻结精修偏好而失败；`.artifacts` 点目录也未被默认上传。DB1 现显式冻结精修偏好并分别核对首次稳定转写/独立精修稿，workflow 显式包含 hidden files 且无文件 fail closed；该修复与远端 CI 闭环仍为实现完成·尚未验收。正式应用 `userData` 和干净机 I4 尚未达到实机验收完成。 |
...
2026-08-03 当前工作树为 core 410/410、integration 27/27、evidence 165/165，共 602/602；另有当前源码对应的 `caption-layout-report@v2` 真实 Chromium 布局资格 15/15 场景、16/16 不变量，包含 identity-only 视口淘汰回报、容量保险墓碑及迟到高版本 `final` 不复活，报告边界仍明确保持 `humanVisualReview=false`、`dpiMatrix=false`、`audioCapture=false`。可见 DWM 资格基础设施的 9 项契约测试已计入 evidence；由于当前尚无 36 组合人工观察矩阵，它仍为实现完成·尚未验收。run `30760407160` 已通过 Electron runtime、字幕布局与 DB0，随后暴露 DB1 fixture/独立精修版本漂移及 hidden artifact 未上传；当前修复仍待新 revision 复验。Windows 系统 `tar.exe` 与 Electron child 在受限沙箱内可能被 `EPERM` 拒绝，相关 lane 必须在允许启动这些精确子进程的 Windows runner/本机运行；这属于执行环境前提，不能把沙箱失败计作产品断言通过或失败。
