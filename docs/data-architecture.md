...
| `agent_model_profiles` / `agent_model_purpose_assignments`（目标） | 模型配置与用户用途 | `profile_id, revision, label, adapter_id, api_style, https_origin, model, catalog_revision, pricing_json, credential_slot_id`; `purpose, profile_id, model, expected_profile_revision, updated_at` | 首版 `adapter_id/api_style` 固定为 OpenAI-compatible，DeepSeek 是可修改的预置档案；用户可另建多个档案。`purpose` 只允许默认、信息提取、摘要与总结、分析与规划，内部 recipe 映射不进入 UI。表内只保存非敏感配置和凭据槽引用，不保存 API key；用途专用值为空时读取默认档案。物理表名与字段可在实现 SPEC 中收紧，但事实身份必须由追加 migration 保存。 |
| `agent_model_run_bindings`（目标） | 不可变运行模型事实 | `run_id, purpose, profile_id, profile_revision, adapter_id, https_origin, model, capability_json, budget_json, pricing_revision, created_at` | 每个 `run_id` 冻结一份精确模型运行绑定；重试复用，用户换模型新建 `run_id`。历史比较从此读取模型、配置 revision、能力和价格版本；凭据明文不进入。 |
