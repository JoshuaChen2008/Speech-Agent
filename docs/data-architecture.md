...
| `agent_model_profiles` / `agent_model_profile_models` / `agent_model_purpose_assignments`（目标） | 模型配置与用户用途 | `profile_id, revision, label, adapter_id, api_style, https_origin, base_path, catalog_revision, credential_slot_id`; `profile_id, model_id, capability_json, pricing_override_json`，`UNIQUE(profile_id, model_id)`; `purpose, profile_id, model_id, expected_profile_revision, updated_at` | 首版 `adapter_id/api_style` 固定为 OpenAI-compatible，DeepSeek 是可修改的预置档案；用户可另建多个档案。**一个档案是一个受信任连接（`https_origin` + `base_path` + adapter/apiStyle）加一份凭据加一组 model**：档案行不再持有单个 `model`，模型清单独立成表，`(profile_id, model_id)` 是精确二元组。`https_origin` 只保存 scheme+host+port 供 exact origin 校验与 redirect 拒绝；`base_path` 默认 `/v1`，必须是不含查询、片段与 `..` 的绝对路径。`capability_json` 是恰好六字段闭集 `maxInputTokens, maxOutputTokens, supportsToolCalling, supportsStructuredOutput, supportsStreaming, usageReporting`，由用户声明或静态预置目录提供，接入层不猜测、不探测。`purpose` 只允许默认、信息提取、摘要与总结、分析与规划，四者必须能各自绑定不同 `(profile_id, model_id)`，内部 recipe 映射不进入 UI。表内只保存非敏感配置和凭据槽引用，不保存 API key；**一档案一槽，不跨档案共享，即使两个档案指向同一 origin**，删除档案在同一事务内删除其模型清单与凭据槽。用途专用值为空时回落到默认档案，且公开投影必须显式标注这是回落而非单独配置；默认也为空时投影为 `provider_not_configured`。`catalog_revision` 只由用户编辑模型清单推进，首版不做自动远端目录刷新。物理表名与字段可在实现 SPEC 中收紧，但事实身份必须由追加 migration 保存。 |
| `agent_model_run_bindings`（目标） | 不可变运行模型事实 | `run_id, purpose, profile_id, profile_revision, adapter_id, api_style, https_origin, base_path, model_id, capability_json, budget_json, pricing_revision, pricing_source, credential_slot_id, created_at` | 每个 `run_id` 冻结一份精确模型运行绑定；重试复用，用户换模型新建 `run_id`。绑定行写入后不可改写，删除或修改配置档案只影响未来运行，既有绑定与产物保留不可变模型身份。绑定在运行创建期由接入层按固定顺序解析：recipe → 用途（静态全映射）→ 用途指派 → 回落默认 → 档案与 model → 能力校验 → 预算推导 → 价格解析 → 凭据槽解析，任一步失败即 fail closed 且不进入下一步；调用方只给 recipe 身份与已判定的执行形态，不给档案、model、URL、header、预算或凭据。`budget_json` 必须承载第 4.3 节十轴预算的全部数值，其中单次请求输入 token 轴由 `capability_json.maxInputTokens` 推导，累计计费输出 token 轴受 `maxOutputTokens` 约束。`pricing_source` 与 `pricing_revision` 成对，只允许 `static_catalog` / `profile_override` / 同时为空；同时为空表示无已登记单价，费用估算固定为空并标记为估算，不得显示 0。`credential_slot_id` 是不透明槽 ID，不是密文也不是密钥引用路径，用于保证运行期间档案被改动或删除后借用仍可复现；槽已不存在时收束为 `AGENT_PROVIDER_AUTH_FAILED`。历史比较从此读取模型、配置 revision、能力和价格版本；凭据明文不进入。 |
| `formal_agent_interactions`（目标） | 正式 Agent 交互结果 | `interaction_id, run_id, recipe_id, recipe_version, scope_json, scope_digest, input_digest, model_binding_id, execution_form, escalation_reason, terminal_reason, usage_json, cost_estimate_json, comparison_group_id, result_json, result_digest, created_at, terminal_at` | 终态只保存最小范围/模型身份与最终结果；活动恢复所需提示在交互记忆信号提取后清理并只留 digest。`execution_form` 只允许 `single_shot` 或 `agent_loop`，与 `escalation_reason` 一起在运行创建期冻结，运行中不得改写。`terminal_reason` 覆盖成功、失败与协程式取消；取消同为终态，允许结果为空但不得补造。`usage_json` 必须带用量来源（provider 返回或确定性估算）。`cost_estimate_json` 在运行收束时按 `agent_model_run_bindings.pricing_revision` 计算一次并冻结，历史与导出一律不重算，单价缺失时为空且标记为估算，`pricing_revision` 不在本表重复。`comparison_group_id` 对同一 `(scope_digest, input_digest)` 稳定，用于同源换模型比价。删除交互必须级联删除工具调用记录，不删除其明确产生且已经独立摄取的个人记忆事实。SEM-F35 导出从一个终态交互及其关联模型绑定/工具记录读取一致快照，不新增导出副本表。 |
| `formal_agent_tool_calls`（目标） | 完整工具调用审计 | `call_id, interaction_id, attempt, call_order, tool_name, schema_version, started_offset_ms, ended_offset_ms, status, error_code, args_json, args_digest, result_json, result_digest, source_refs_json, counts_json` | `UNIQUE(interaction_id, attempt, call_order)` 形成全序；自动重试建立新 `attempt`，旧 attempt 的记录一律保留，不得删除、覆盖或合并。`error_code` 只使用独立于 `agent_jobs` 任务错误码的工具错误码闭集 `TOOL_ARGS_INVALID`、`TOOL_SCOPE_DENIED`、`TOOL_NOT_AVAILABLE_FOR_RECIPE`、`TOOL_BUDGET_EXCEEDED`、`TOOL_TIMEOUT`、`TOOL_CANCELLED`、`TOOL_INTERNAL_FAILURE`。`args_json/result_json` 保存工具 exact Schema 校验且预算内的完整结构化正文，不静默截断；单次调用 `args` 不超过 8 KiB、`result` 不超过已登记的 `maxResultBytes`（上限 64 KiB），一次运行累计工具结果字节不超过 256 KiB、其中累计来源正文字节不超过 128 KiB。模型在工具调用之外产生的中间 assistant 文本零持久化。UI 默认折叠并允许用户展开。工具接口禁止凭据、现场音频、音频路径、本地绝对路径和内部思维过程；正文不得复制进普通日志、报告、个人记忆或测试证据 JSON。 |
...
| `agent_debug_threads` / `agent_debug_messages`（旧实现留存，新设计不写） | 本地调试聊天 | `thread_id, selected_session_id, selected_input_watermark, selected_transcript_version, selected_input_digest, created_at`; `message_id, thread_id, role, content_json, provider, model, created_at` | 调试聊天独立于字幕会话、Agent 产物和个人记忆；选择上下文时冻结完整输入身份，只保存在本地，可清空，永不自动进入记忆或确认关键词。内部思维过程不得写入。 |
