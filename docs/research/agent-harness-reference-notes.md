...
可直接复用的是 `@earendil-works/pi-ai` 的小集合接口、所需 provider factory、`createProvider()`、精确模型查询、请求取消，以及 `fauxProvider()` 的确定性 provider 测试替身（只在测试构建注册）；若采用 Pi loop，则只把 `models.streamSimple.bind(models)` 注入 `@earendil-works/pi-agent-core`。不应整段复制的是全量 `builtinModels()`、将被移除的 `/compat` 全局 API、coding-agent 的 `ModelRuntime` / `ModelRegistry`、`models.json` 热加载、`auth.json` / OAuth 登录 UI 和 home-dir 约定；它们把命令行产品、文件布局与广泛凭据发现耦合在一起。

> **2026-08-29 收窄（M8）**：`envApiKeyAuth()` 已从上述可复用清单移出，改为**明确禁用**。它从 `process.env` 发现凭据，与 `SEM-F33` 的「凭据只经主进程 `safeStorage` 按档案保存、Agent utility 只取本次调用的有界副本」直接冲突，也会绕过本段末尾自己提出的那条边界。取代方案是只读取本次调用副本的自有 auth resolver。同一轮还禁用 `prepareNextTurn` 的运行中模型替换路径——不实现它即得到「整次运行模型固定」这一不变量。[Migration away from global compat](https://github.com/earendil-works/pi/blob/main/packages/ai/README.md#migrating-from-the-old-global-api) · [coding-agent model runtime](https://github.com/earendil-works/pi/blob/main/packages/coding-agent/src/core/model-runtime.ts)
