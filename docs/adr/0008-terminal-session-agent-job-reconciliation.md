...
6. Agent 总开关从关闭转为开启时，宿主保存新的 `automaticProcessingSince`；自动对账只处理 `ended_at >= automaticProcessingSince` 的终态会话。自动请求遇到更早会话时返回 `outside_automatic_window`，不会在重新开启后静默处理关闭期间或更早的历史会话。Agent 模型 provider 配置、凭据、本地模型就绪或云端披露暂不满足时不创建任务；资格恢复后只对上述时间边界内的会话重新对账。用户仍可从历史明确请求更早终态会话，且请求必须重新通过当前 Agent 处理资格校验。
