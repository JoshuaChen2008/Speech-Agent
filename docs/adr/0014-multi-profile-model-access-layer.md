...
2. **一个配置档案 = 一个受信任连接 + 一份凭据 + 一组 model。** API base URL 由用户提交的 canonical HTTPS origin 与基础路径（默认 `/v1`）组成；模型清单独立于档案行，`(档案 ID, model ID)` 是精确二元组。首次 v6 初始化只播种可修改、可删除的 `deepseek-openai-template@1` provider 模板：`https://api.deepseek.com` + `/`、空 model、空用途、凭据 absent；它不是产品绑定，显式删除后不得重建。
...
4. 能力是**六字段闭集**，按 model 声明：`maxInputTokens`、`maxOutputTokens`、`supportsToolCalling`、`supportsStructuredOutput`、`supportsStreaming`、`usageReporting`。六字段配置事实只能由用户完整确认并通过 `addModel/updateModel` 写入；provider 模板与远端目录只能返回可空建议，不得自动落库。`deepseek-openai-template@1` 基于 2026-08-30 官方快照建议四个布尔为 true、两个 token 上限为 null。只有 `supportsToolCalling` 是硬绑定条件且只对 Agent Loop 成立。
...
10. 不实现自动远端模型目录刷新。只提供用户明确触发的一次性拉取 application adapter，它不扩张三接口；结果状态只允许 `success/revision_conflict/invalid_request/credential_unavailable/redirect_rejected/remote_unavailable`，建议只存在内存，用户确认后才通过九命令写入，拉取本身零写入、零 revision、零 changed。
11. 产品不建立 price、cost、currency、pricing catalog、profile override、pricing revision、费用估算或金额展示。运行比较只保留 exact `ModelUsageV1`：input/output token、`provider/estimated` 用量来源、可空 cache-hit/cache-miss input token，以及满足一致性条件时派生但不持久化的缓存命中率。DeepSeek adapter 映射 `prompt_cache_hit_tokens` 与 `prompt_cache_miss_tokens`；不一致或未知缓存字段全部收束为 null。
...
- 相比让 `model` 保持不透明，六字段能力闭集使十轴预算与升级阈值可推导；代价是用户必须为每个 model 完整确认能力，声明错误会表现为绑定拒绝而不是运行失败——这被判定为更安全的方向。
- 相比支持自动目录刷新，本方案在用户无动作时永不改变配置事实；代价是新模型上线后用户必须手动拉取或手填并确认完整能力。
- 相比直接依赖 Pi 的完整 provider 生态与凭据发现，本方案只依赖公开小接口；代价是需要自己实现 auth resolver、能力校验、预算推导与 token/cache 归一化，换来凭据边界与配置持久化完全由本产品拥有。
...
- 建立价格目录、单价覆盖、pricing revision、费用估算或金额展示。
