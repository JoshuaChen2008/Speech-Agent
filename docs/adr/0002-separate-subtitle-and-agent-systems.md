...
Agent runtime 通过本项目的可替换 `AgentRuntime` 边界接入；复用 Pi 低层 Agent Core/Loop 的方向已确认，但必须先通过 A1 兼容探针，coding-agent 的默认 shell/文件工具和会话存储不成为产品权威。可靠消费采用事务 outbox 还是 durable cursor 在 A1 探针后冻结。FTS5 按历史搜索需求另行增加，embedding/`sqlite-vec` 明确 Deferred，不阻断 SQLite 历史、字幕 MVP 或首版 Agent 摘要。
