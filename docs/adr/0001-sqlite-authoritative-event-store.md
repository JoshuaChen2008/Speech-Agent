...
选择存储方案时需要保留现有事件语义：一遍定稿不能因后续精修丢失，迟到与重试不能制造重复正文，AI 或向量能力失败不能影响本地字幕。
...
4. 使用事务将“字幕事件写入、当前段落投影更新”原子提交。Agent 可靠唤醒采用事务 outbox 还是 durable cursor 由 A1 冻结；无论哪种都不能让 Agent job 失败回滚字幕事实。
...
- 会话事实、当前段落、摘要输入和索引任务可在同一事务边界内保持一致。
- 历史、全文和向量检索不需要扫描多个会话文件。
- worker 崩溃后可重放 outbox，AI/embedding 故障与实时字幕解耦。
