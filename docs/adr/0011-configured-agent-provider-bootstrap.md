...
- 状态：已被 [ADR 0014](0014-multi-profile-model-access-layer.md) 取代（2026-08-29）
- 日期：2026-08-10
- 决策者：项目负责人
- 依赖：ADR 0002、ADR 0003、ADR 0005、ADR 0009
- 取代说明：ADR 0014 整体取代本 ADR 的 provider 形状——单条目配置表改为多档案加每档案多 model，启动环境变量凭据改为按档案的主进程 `safeStorage` 槽，写死的 `https://api.deepseek.com` 改为 exact 校验的任意 HTTPS origin 加独立 `base_path`，不透明 `model` 加配置表预算改为六字段能力闭集加十轴预算。下文保留原文以记录当时的决策理由与 D9/D10/D11/D14 已实现子边界的可追溯依据。仍继续有效的部分已由 ADR 0014 逐项重新登记：第 5 项凭据借用生命周期（第 8 项，失效范围收窄为单个档案槽）、第 8 项产品设置事实与 `SETTINGS_REVISION_CONFLICT`（第 14 项）、第 9 项替身必须经真实路径（第 15 项，并收紧为生产不可达）、D9 的环境净化规则（第 7 项，降级为纯加固不变量）、以及 D9/D10/D14 的可靠性不变量清单（第 16 项）。
