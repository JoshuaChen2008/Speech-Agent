...
- `.tb-hole` 由当前停靠几何推导的 `588 × 64` 只是在首帧、reload、布局未就绪或非法报告时的临时 fail-safe；有效同代矩形到达后必须立即收缩到工具条真实轮廓。
...
工具条脱离字幕背景的锁定态只能从两列三行六点握把开始拖动，命中盒保持 `24 × 30 DIP`；此时只移动工具条。未锁定嵌入态不显示握把，握把不占布局、不进入真实轮廓且不能发起工具条拖动，字幕窗与工具条组合只从字幕卡普通拖动区移动。按钮、状态文字和控件间隙始终不是隐藏拖动区域。
