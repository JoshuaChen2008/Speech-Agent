...
## 13. UX-2 交付 · Agent Bar：范围、资格、运行与最小交互历史（S3 / J22·J24 Core 子边界）

> 交付类型：`UX design` · 2026-08-31 · 覆盖表面只有 §5.3「Agent Bar」与 §5.4「交互历史、工具调用记录与导出」中属于 Agent Bar 侧的最小交互历史部分。
>
> 本节是流程、信息架构、状态矩阵、文案与可访问性说明，**不构成** renderer 实现、fixture preview 或任何 J 旅程证据；本轮未改动任何 `src/**` 文件，也未产生预览页面。J22/J24 状态不因本节变化。
>
> **前置核查（2026-08-31）**：`src/agent/contracts/` 目前只有 `agent-context-ui.js`、`agent-model-ui/v1.0.0.js`、`recipes.js`、`budget-axes.js`、`model-access-core.js`、`personal-context-core.js`；无任何 `agent-run:*` UI contract 或 fixture。`src/main/ipc/channels.js` 无 `agent-run:*` 六频道，角色闭集无 `agent`，`src/preload/` 无 `agent.js`，`src/agent-window/**` 不存在。`openspec/changes/implement-agent-redesign-s3/tasks.md` 188 项 task 尚未勾选任何一项。因此本轮只能交付 `UX-2` 设计与 contract requests（§2.2 交接门：`UX-2` 进入条件"S3/S4 contract 方向已登记"已满足；`UX-3` 进入条件"对应 Core contract、错误码与 fixture 已冻结"未满足）。`recipes.js`/`budget-axes.js` 是 Core 内部合同，不是 renderer-facing 投影，本节不消费它们，也不把 OpenSpec 文字规格当作已冻结运行时 API。
>
> 不覆盖：设置 · 个人上下文管理（S1，见 §11）、设置 · Agent 模型配置档案（S2，见 §12）、完整交互历史/工具调用记录/单交互导出（S4/S5，字幕历史窗）。

### 13.1 信息架构与组件边界

Agent Bar 位于新 `agent` 角色窗口（可聚焦、不穿透、非模态；窗口生命周期与几何合同归 Core）：

```text
Agent Bar（agent 窗口）
├─ 范围条
│  └─ 当前选区 / 终态会话 / 日期范围 / 项目（单选一种范围类型）
├─ 意图输入与提交
│  ├─ Agent 处理资格提示（非 ready 时显示原因与下一动作，替代输入框可提交状态）
│  ├─ 自然语言意图输入框
│  └─ 提交 / 取消控件
├─ 结果表面
│  ├─ 收敛结果的产品语言标签（不暴露 recipe ID / confidence / routing_mode）+ 改选入口
│  ├─ 最终结果正文
│  ├─ 结果头部：范围可读标签 · 模型运行身份 · 相对时长 · 用量（已知/未知）· 缓存命中率（已知/未知）
│  └─ 反馈动作：编辑 / 接受 / 拒绝 / 记住 / 忘记
└─ 最近交互（有界列表）
   └─ 时间戳 · 范围/模型身份 · 终态 · 点击进入字幕历史对应详情
```

三条边界裁决：

1. **Agent Bar 只承载"当前这一次交互 + 最近若干条"**；完整交互历史、终态详情、工具调用记录与单交互导出留在字幕历史窗（§5.4），本轮不在 Agent Bar 内复制第二套历史列表。
2. **S3 阶段工具区入口缺席，不是禁用按钮。** 工具调用记录、多 attempt、预算耗尽属 S4（handoff §3 表格：`S4 Agent Loop` 行）；提前呈现即把 S4 能力伪装成 S3 能力，是任务明令禁止项。
3. **正式 renderer 根路径尚未登记。** 按 §4.1，建议使用 `src/agent-window/**`，但须由 Core owner 在实现前正式登记；本节只做组件边界命名，不落地任何路径或文件。

界面永不出现：recipe ID、`routing_mode`、confidence、轮次上限、工具授权、执行形态、升级理由、内部思维过程、提示正文、API key、凭据槽、adapter/factory、IPC channel、本地路径、任何金额字段（SEM-F16/F28/F31/F33、§5.3/§5.4/§8）。

### 13.2 端到端流程

| # | 流程 | 步骤 |
|---|---|---|
| F0 | 载入与恢复 | Agent Bar 窗口获得焦点 → 先订阅 `agent-run:changed` → 再读当前资格与最近交互 snapshot → 记住 revision。此后只接受更高 revision，较旧值直接丢弃。订阅失败或 snapshot 字段缺失 → 整个表面进入只读不可用，不猜测原因。 |
| F1 | 选择范围 | 四选一：当前选区 / 终态会话 / 日期范围 / 项目。切换范围类型清空未提交的收敛结果与草稿意图之外的其它表面状态；不清空用户已输入的自然语言意图文本。 |
| F2 | 查看资格 | 范围确定后立即请求资格；非 `ready` 时输入框仍可编辑但提交控件禁用，显示原因文案与（如已提供）下一动作。 |
| F3 | 提交意图 | 用户输入一次自然语言意图 → 点击提交 → 控件进入 pending，范围与输入框锁定但内容保留。 |
| F4 | 收敛呈现与改选 | 收敛完成后，结果表面显示产品语言标签（例如"问答""分析报告""规划建议""文本改写"等，映射自 recipe 的用户可见类别，不显示 recipe ID）与改选入口；用户改选按"取消当前运行 + 新建运行"处理，产生新的运行与结果，旧结果仍可从最近交互回看。 |
| F5 | 运行中 | pending 态持续到终态；用户可点击"取消"进入取消请求中。 |
| F6 | 取消 | 取消请求中 → 等待权威终态 → 取消终态（结果可能为空，已发生的部分不补造）。取消请求中**不得**提前显示"已取消"。 |
| F7 | 终态呈现 | 成功：结果正文 + 结果头部事实。失败：错误原因与（如提供）下一动作，不猜测可重试性。结果为空：显式空态说明，不同于加载中或失败。 |
| F8 | 反馈动作 | 编辑 / 接受 / 拒绝 / 记住 / 忘记均先进入 pending（按钮禁用 + `aria-busy`）→ 等待 CommandResult 才呈现新状态；冲突或失败保留用户已做的编辑。 |
| F9 | 最近交互 | 结果表面下方显示有界列表，按终态时间倒序；触达上界显示"还有更多"并可续读，不伪造完整总数。列表项点击跳转字幕历史对应详情，不在 Agent Bar 内展开工具记录。 |
| F10 | 报告自动呈现 | 用户在设置开启报告自动呈现偏好后，满足资格的终态会话结束时，Agent Bar（或其非模态结果表面）至多呈现一次会后结构化纪要；renderer reload、重复停止、重复通知只恢复同一权威 run，不重复呈现。 |

流程不变量：写动作一律先 pending 再回执，**无乐观成功态**；成功只能来自 Core 的 snapshot 或 CommandResult；错误码、可重试性、下一动作、终态理由均由 Core 提供；界面不从等待时长、异常字符串、ID 形态、DOM 顺序或缺字段推断状态。
...
| 状态 | 表现 |
|---|---|
| 当前选区可用 | 显示选区摘要（来源：字幕历史，跨窗口身份传递方式见 `AUI-CR-011`） |
| 当前选区不可用 | 入口显示为不可选，说明"请先在字幕历史选择内容" |
| 终态会话/日期范围/项目清单为空 | 该范围类型显示空状态说明，不提供无法选出结果的下拉框 |
| 范围含省略标记 | 提交后的结果头部显式标注省略事实，不静默隐藏 |
| 范围投影不可用 | 整个范围条只读不可用 + 通用说明 + 重试 |

**B 九值 Agent 处理资格**（Core 按固定顺序计算，UI 只翻译原因与下一动作）

| 资格值 | 表现 |
|---|---|
| `ready` | 提交控件可用 |
| `no_committed_transcript` | 说明"所选范围内还没有已提交的字幕内容"，提交禁用 |
| `outside_automatic_window` | 说明"该会话在自动处理边界之前"，同时说明用户仍可从历史明确请求（`SEM-F28`：用户请求跳过自动窗口但不绕过其它条件） |
| `agent_disabled` | 说明"Agent 功能当前未开启"+ 指向设置入口 |
| `provider_not_configured` | 说明"还没有配置可用的模型"+ 指向设置 · Agent 模型配置档案 |
| `cloud_disclosure_required` | 说明需要云端处理披露确认，提供确认入口（入口本身待 `AUI-CR-012`） |
| `credential_unavailable` | 说明"所选模型的凭据当前不可用"+ 指向设置 |
| `local_model_not_ready` | 说明"本地模型尚未就绪"+ 指向模型资源设置 |
| `session_not_terminal` | 说明"所选会话仍在进行中" |
| 未知资格值 | fail closed：入口不可提交，保留范围和用户输入，显示通用不可运行说明，不猜测可重试性 |

**C 运行生命周期**

| 状态 | 表现 |
|---|---|
| 提交中（尚未收到 pending 确认） | 提交按钮禁用 + `aria-busy` |
| pending（运行中） | 范围与输入框锁定但内容可见；显示"正在处理"+ 取消入口 |
| 取消请求中 | 显示"正在取消"，不显示已取消 |
| 取消终态 | 显示取消已完成 + 已发生的部分结果（如有），不补造缺失部分 |
| 成功终态 | 结果表面按 §13.2 F7 呈现 |
| 失败终态 | 错误原因 + 下一动作（如有） |
| 结果为空 | 显式空态说明，与失败/加载中视觉区分 |

**D 结果头部事实**

| 状态 | 表现 |
|---|---|
| 范围可读标签 | 始终显示（例如"本次会话""2026-08-24 至 2026-08-30"） |
| 模型运行身份已知 | 显示可展示的模型身份投影（非内部 adapter/API key） |
| 相对时长 | 始终显示（如"用时 12 秒"），不显示绝对时刻 |
| 用量已知 | 显示 input/output token，来源恒为 `provider` |
| 用量未知 | 显示「用量未知」文案，**不出现任何数字，不估算** |
| 缓存命中率已知 | 显示百分比 |
| 缓存命中率未知 | 不显示该行，不显示占位 0% |
| 任何金额字段 | 永不出现 |
...
| 状态 | 表现 |
|---|---|
| renderer reload | 重走"订阅 → snapshot → revision"，不恢复本地缓存的成功态 |
| 迟到的较旧 revision | 静默丢弃，界面不闪回 |
| 迟到的运行结果（已取消后到达） | 不改写已收束的取消终态 |
| 未知 contract 版本 | exact validator 拒绝整个载荷，该表面进入只读不可用并可重试 |
| 未知枚举值（资格/终态理由/routing 标签等） | 同上，fail closed，不猜测含义 |
| 缺失字段 | 同上，不从部分字段继续渲染 |
...
|---|---|
| 列表为空 | 空状态说明"还没有交互记录" |
| 列表触达上界 | 显示"还有更多"，提供续读，不伪造完整总数 |
| 反馈动作 pending | 对应按钮禁用 + `aria-busy` |
| 反馈成功回执 | `role="status"` 文本 |
| 反馈 revision conflict | `role="alert"`：说明未写入、保留用户编辑 |
| 反馈失败 | `role="alert"`：具体原因 |
| 历史列表 | 不含未读状态、角标、红点或计数 |

**G 报告自动呈现**

| 状态 | 表现 |
|---|---|
| 偏好默认关闭 | 终态会话不自动创建或呈现报告 |
| 偏好开启 | 满足资格的终态会话至多非模态呈现一次 |
| reload/重复停止/重复通知 | 只恢复同一权威 run，不生成第二份呈现 |
| 偏好关闭 | 不删除旧报告 |
| 呈现表面 | 不使用系统通知、模态框、声音或抢焦点；无未读标记 |

### 13.4 文案

规范术语固定为：Agent Bar、正式 Agent 交互、个人上下文包、会话经历记录、个人记忆、来源引用、模型运行绑定、相对时长、用量未知。禁止"AI 助手""智能分析""已学习""清除记忆"及任何无修饰的"完成""可用""测试通过"。

| 位置 | 文案 |
|---|---|
| 窗口标题 | Agent |
| 范围条标签 | 当前选区 / 终态会话 / 日期范围 / 项目 |
| 选区不可用 | 请先在字幕历史选择内容 |
| 资格：no_committed_transcript | 所选范围内还没有已提交的字幕内容。 |
| 资格：outside_automatic_window | 该会话早于自动处理边界；你仍可明确请求处理它。 |
| 资格：agent_disabled | Agent 功能当前未开启。前往设置开启。 |
| 资格：provider_not_configured | 还没有配置可用的模型。前往设置配置模型。 |
| 资格：cloud_disclosure_required | 这次处理会使用云端模型，需要你先确认。 |
| 资格：credential_unavailable | 所选模型的凭据当前不可用。前往设置检查凭据。 |
| 资格：local_model_not_ready | 本地模型尚未就绪。前往设置查看模型资源。 |
| 资格：session_not_terminal | 所选会话仍在进行中。 |
| 资格：未知值 | 当前无法处理这个请求。 |
| 提交动作 | 发送 |
| pending | 正在处理，请稍候。 |
| 取消动作 | 取消 |
...
| 反馈 pending | 正在提交，请稍候。 |
| 反馈成功 | 已保存。 |
| 反馈冲突 | 这条内容已在别处更新，本次未写入。你的编辑仍保留。 |
| 结果为空 | 这次没有生成结果。 |
| 最近交互空态 | 还没有交互记录。 |
| 上界提示 | 还有更多记录未载入。 |
| 通用不可用 | Agent Bar 暂时不可用。 |
| 未知值降级 | 这条记录包含当前版本无法解释的内容，暂不提供操作。 |

### 13.5 可访问性与视觉

- **主通道是文字与形状，颜色只作冗余通道。** 资格原因、生命周期状态、用量已知/未知都先有文字标签；把颜色全部去掉后仍可区分。不使用红点、角标、未读计数。
- **深色与浅色**在 `src/ui/shared/tokens.css` 的语义层切换，组件样式里不出现 `[data-theme]` 分支；不新建 Agent 专属发光、渐变或玻璃表面，延续 `current-ui-ux-handoff.md` §3.1 的视觉基线。
- **Windows 系统高对比**由 `forced-colors` 基线接管颜色；状态区分不依赖颜色，接管后不丢失任何区分。
- **键盘 focus**：范围类型选择是 `radiogroup`；提交、取消、改选、反馈动作全部可达；确认层（如需要）获得焦点并可 `Esc` 取消，关闭后焦点回到触发控件；禁用控件保留可及名称与禁用理由。
- **屏幕阅读器名称**：pending 用 `aria-busy`；成功用 `role="status"` 的 `aria-live="polite"`；冲突与失败用 `role="alert"`；反馈动作的可及名称写出作用对象（"记住这条回答"而非裸"记住"）。
- **reduced motion**：只有 pending 存在过渡指示，且在 `prefers-reduced-motion` 下降级为静态文字；运行中不使用无限"智能"动画。
- **隐私**：API key、凭据槽、受信任 origin 校验细节、IPC channel、本地绝对路径、设备名、provider 原始事件、音频与音频路径都不进入本界面，也不进入其可及名称。

### 13.6 展示事实映射表

该表同时是 UX-3 的启动检查表：Core 签发对应 contract 后，逐行核对来源列即可判断 renderer 能否开工。

| 展示事实 | 来源 |
|---|---|
| Agent Bar 是新 `agent` 角色窗口，可聚焦、不穿透、非模态 | 已决定 · `agent-redesign-execution-plan.md` §阶段 F 第 1 条 |
| 范围四值：当前选区 / 终态会话 / 日期范围 / 项目 | 已决定 · `CONTEXT.md`「Agent Bar」词条；`AUI-CR-010`（可选项与冻结身份投影未签发） |
| 「当前选区」跨窗口身份传递 | `AUI-CR-011`（未签发） |
| 九值 Agent 处理资格闭集与固定优先级 | 已决定 · `SEM-F28`；openspec S3 spec「Agent 处理资格必须由 main 按固定顺序计算」 |
| 资格原因的产品文案与下一动作 | `AUI-CR-012`（下一动作闭集未签发，本节文案是设计建议非事实） |
| 运行生命周期（pending/取消请求中/取消终态/成功/失败/结果为空） | `AUI-CR-013`（状态闭集与转换事件未签发） |
| 收敛结果产品语言标签与改选 | 已决定 · `SEM-F28`「收敛结果必须以产品语言呈现并允许用户改选」；`AUI-CR-015`（标签集合未签发） |
| 结果头部：范围/模型身份/相对时长/用量/缓存命中率 | 已决定 · `SEM-F31` §5.3 第 5 条；`AUI-CR-014`（exact 投影未签发） |
| 用量未知显示「用量未知」、零估算 | 已决定 · `SEM-F28`「provider 未返回 usage 时不估算、不编造数字」；`testing-strategy.md` S3 子边界表 |
| 反馈动作闭集：编辑/接受/拒绝/记住/忘记 | 已决定 · `SEM-F32`；`AUI-CR-017`（exact 命令与回执未签发） |
| 最近交互有界列表、keyset 续读、排除 `intent.route` | 已决定 · openspec S3 spec「交互历史与详情必须使用 keyset 分页和最小投影」；`AUI-CR-016`（Agent Bar 侧有界投影未签发） |
| 报告自动呈现偏好默认关闭、每会话至多一次 | 已决定 · `SEM-F28`；`AUI-CR-018`（读写投影与呈现回执未签发） |
| `changed` 事件按单调 revision 恢复，旧值丢弃 | 已决定 · openspec S3 spec「六个 Agent run IPC」；`AUI-CR-019`（`agent-run:changed` 未签发） |
| 未知 contract 版本/枚举/字段 fail closed | 已决定 · `agent-ui-ux-handoff.md` §7；openspec S3 spec「S3 fixture 必须脱敏、版本化」 |
| 界面不展示 recipe ID/routing_mode/confidence/轮次/工具授权/执行形态/升级理由 | 已决定 · `SEM-F16`/`SEM-F28`；`agent-ui-ux-handoff.md` §5.3 |
| S3 阶段无工具调用记录/多 attempt/预算耗尽入口 | 已决定 · `agent-redesign-execution-plan.md` 阶段 D/E 划分；`testing-strategy.md` S3 Core 子边界表 |

### 13.7 本轮不构成

- 不构成 renderer 实现：本轮未创建 `src/agent-window/**` 或任何 `src/**` 文件，未产生 fixture preview 页面。
